...
use Flexio\Modulo\Colaboradores\Models\ColaboradoresDistribucionSalario;
...
class Colaboradores extends CRM_Controller {
    ...
    public function listar() {
        ...
        $breadcrumb["menu"] = array(
            "url" => $this->auth->has_permission('acceso', 'colaboradores/crear') ? 'colaboradores/crear' : '#',
            "nombre" => $this->auth->has_permission('acceso', 'colaboradores/crear') ? "Crear" : '&nbsp;',
        );
        ...
        if ($this->auth->has_permission('acceso', 'colaboradores/listar')) {
    ...
    function crear($colaborador_uuid = NULL) {
        ...
        if ($colaborador_uuid != NULL) {
            $colaborador_info = Colaboradores_orm::with(array('distribucion', 'estado', 'dependientes', 'familia', 'deducciones', 'estudios', 'experiencia', 'beneficiario_principal' => function($query) {
    ...
    private function guardar_colaborador($colaborador_uuid = NULL) {
        ...
        try {
            ...
            $datos = $_POST["campo"];
            $fieldset['islr_gasto_representacion']=$fieldset['ss_gasto_representacion']=0;
            if (isset($fieldset['gasto_de_representacion']) && doubleval($fieldset['gasto_de_representacion']) > 0) {
                $percent = doubleval($fieldset['gasto_de_representacion']) <= 15000 ? 10 : 25;
                $fieldset['islr_gasto_representacion'] = (doubleval($fieldset['gasto_de_representacion']) * $percent) / 100;
                // id 2 = seguro social
                $seguro=Deducciones::where("id", "=", 2)->first();
                if($seguro){
                    $percent= $seguro->rata_colaborador;
                    $fieldset['ss_gasto_representacion'] = (doubleval($fieldset['gasto_de_representacion']) * $percent) / 100;
                }
            }
            ...
            //Procesa Datos de Distribucion de Salario
            if(!empty($_POST['distribucion'])){
                //Verificar si esta vacio
                if (Util::is_array_empty($_POST["distribucion"]) == false) {
                    //Verificar si existe en la base de datos 
                    $distribucion_salario = ColaboradoresDistribucionSalario::where('colaborador_id', '=', $colaborador->id);
                    
                    if(count($distribucion_salario->get())==0){
                        $postData = [];
                        
                        foreach($_POST['distribucion'] as $data){
                            $postData[] = new ColaboradoresDistribucionSalario($data);
                        }
                       
                        $colaborador->distribucion_salario()->saveMany($postData);
                       
                    } elseif (count($distribucion_salario->get())>0){
                        //borrar actuales valores
                        $colaborador->distribucion_salario()->delete();
            ...
            //Salvar Distribucion de empleados
