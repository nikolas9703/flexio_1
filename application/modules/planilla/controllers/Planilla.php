 ...
 class Planilla extends CRM_Controller{
	...
	function __construct() {
     ...
     function crear($planilla_uuid = NULL) {
     	...
     	$data['ciclos'] = Catalogos_orm::where("identificador","=",'Ciclo')->where("activo","=",1)->get();
     	$data['tipo_planilla'] = Catalogos_orm::where("identificador","=",'Tipo Planilla')->where("activo","=",1)->get();
     	...
     	if($planilla_uuid!=NULL){ //Ver Detalles
     		...
     		$id_planilla = array(
     				'uuid_planilla'  => $planilla_uuid
    		);
      		$this->session->set_userdata($id_planilla);
     		...
     		$planilla_info = Planilla_orm::with(array('estado', 'acumulados', 'deducciones','centro','subcentro','area_negocios','pasivos','tipo','licencias','vacaciones','liquidaciones'))
     		->where(Capsule::raw("HEX(uuid_planilla)"), "=", $planilla_uuid)->get()->toArray();
      		$fecha1 =  date("d/m/Y", strtotime($planilla_info[0]['rango_fecha1']));
     		$fecha2 =  date("d/m/Y", strtotime($planilla_info[0]['rango_fecha2']));
     		...
     		//Se usa en el subgrid, Centro Contable
     		if(isset($planilla_info[0]['centro'])){
     			$data['centro_contable'][] = array(
     					"id"=>$planilla_info[0]['centro']['id'],
     					"nombre"=>$planilla_info[0]['centro']['nombre']
     			);
     		}else{
     			$data['centro_contable'][] = array(
     					"id"=>0,
     		...
     		$nombre_planilla = $planilla_info[0]['identificador'].$planilla_info[0]['semana'].$planilla_info[0]['ano'].$planilla_info[0]['secuencial'];

     		$estado = $planilla_info[0]['estado']['valor'];

     		if(trim($estado) =='cerrada' || trim($estado) =='anulado'){
     			$permiso_editar = 0; //Se desatciva las opciones
     		}

     		//Colaboradores con que estan creados la planilla
     		$result = Capsule::table('pln_planilla_colaborador as cc')
     		->where('cc.planilla_id', $planilla_info[0]['id'])
     		->distinct()
     		...
     		if($planilla_info[0]['area_negocio'] > 0){
     			$area_negocio_id = isset($planilla_info[0]['area_negocio_id'])?$planilla_info[0]['area_negocio_id']:0;

     			$colaboradores_noactivados = Capsule::table('col_colaboradores as c')
     			->where('c.centro_contable_id',  $planilla_info[0]['centro_contable_id'])
     			->where('c.empresa_id',  $this->empresa_id)
     			->where('c.departamento_id',  $area_negocio_id)
     		...
     		$data["info"]  = $planilla_info[0];
     		$data["info"]['key'] = $nombre_planilla;
     		$data['tipo_planilla_creacion'] = $planilla_info[0]['tipo']['valor'];
            ...
            $planilla_coment->load('comentario_timeline','planillas_asignados');
     		...
     		$this->assets->agregar_var_js(array(
     				"rango1" =>$fecha1,
     				"rango2" => $fecha2,
     				"acumulados" => !empty($planilla_info[0]['acumulados'])?json_encode($planilla_info[0]['acumulados']):json_encode(array()),
     				"deducciones" => !empty($planilla_info[0]['deducciones'])?json_encode($planilla_info[0]['deducciones']):json_encode(array()),
     				"permiso_editar" => $permiso_editar,
     				"cantidad_semanas" => floor($interval/7),
     				"tipo_planilla_creacion" => $planilla_info[0]['tipo']['valor'],
     				"tipo_planilla_id" => $planilla_info[0]['tipo']['id_cat'],
     				"planilla_id" =>!empty($planilla_info[0]['id'])?$planilla_info[0]['id']:0,
     				"estado_planilla" => $planilla_info[0]['estado']['valor'],
                    "coment" =>(isset($planilla_coment->comentario_timeline)) ? $planilla_coment->comentario_timeline : "",
                    "vista" => "ver"
                    //"planilla_id" => $planilla_coment->id
     		));
            ...
            "ruta" => array(
      		...
      		if($data['tipo_planilla_creacion'] == "xiii_mes"  || $data['tipo_planilla_creacion'] == "vacaciones"  || $data['tipo_planilla_creacion'] == "liquidaciones" || $data['tipo_planilla_creacion'] == "licencias" )
     		{
     			if(!empty($planilla_info[0][$data['tipo_planilla_creacion']])){
     				$id_array = array();
     				foreach($planilla_info[0][$data['tipo_planilla_creacion']] as $key=>$value) {
     					$id_array[] = $value['id'];
     				}
      			...
      			if ( $estado == 'validada' ){
     				...
     				if($planilla_info[0]['tipo']['valor'] == "vacaciones"){
     					$breadcrumb["menu"]["opciones"]["#pagarVacacionBtn"] = "Cerrar planilla";
     				}
     				else if($planilla_info[0]['tipo']['valor'] == "liquidaciones"){
     					$breadcrumb["menu"]["opciones"]["#pagarLiquidacionBtn"] = "Cerrar planilla";
     				}
     				else if($planilla_info[0]['tipo']['valor'] == "licencias"){
     					$breadcrumb["menu"]["opciones"]["#pagarLicenciaBtn"] = "Cerrar planilla";
     				}
                                else if($planilla_info[0]['tipo']['valor'] == "xiii_mes"){
        ...
        $this->assets->agregar_css(array(
            ...
            'public/assets/css/plugins/bootstrap/select2.min.css',
     		...
     		$this->session->set_userdata(
     				array("estado_planilla"=>$planilla_info[0]['estado']['valor'])

     		);

     	}else{
     		//echo "Creando planilla nueva";
     		$data['tipo_planilla_creacion'] = 'regular';
     		if(!empty($_POST)){ //Esta es la condicion para crear planillas Vacaciones/Liquidaciones
     			...
     			if(!empty($_POST['vacaciones'])){
     				...
     				$data['tipo_planilla_creacion'] = 'vacaciones';
     			}
     			else if(!empty($_POST['liquidaciones'])){
     				$data['lista_seleccionada'] = $_POST;
     				$data['tipo_planilla_creacion'] = 'liquidaciones';
     			}
     			else if(!empty($_POST['licencias'])){
     				$data['lista_seleccionada'] = $_POST;
     				$data['tipo_planilla_creacion'] = 'licencias';
     			}
     		}

     		$getIdTipoPlanilla = Catalogos_orm::where("valor","=",$data['tipo_planilla_creacion'])->where("identificador","=",'Tipo Planilla')->where("activo","=",1)->get();
     		$getIdTipoPlanilla = isset($getIdTipoPlanilla[0]->id_cat)?$getIdTipoPlanilla[0]->id_cat:'0';

     		$clause= array('empresa_id' => $this->empresa_id,'estado'=>'Activo');
     		$centros = Centros_orm::listar($clause);
     		$centro_contables= $this->buildTree( $centros );
     		$data['centro_contables'] = $centro_contables;

     		$clause = array("empresa_id" =>  $this->empresa_id);


     		$this->assets->agregar_var_js(array(
     				"tipo_planilla_creacion" => $data['tipo_planilla_creacion'],
     				"tipo_planilla_id" => $getIdTipoPlanilla
     		));
     		$key = "";
      ...
      /*$planilla_info = $this->planillaRepository->find(22);
     ...
     public function ajax_listar_colaboradores_ciclo_pago() {
     	...
     	$empresa = Empresa_orm::findByUuid($uuid_empresa);
      	...
      	$response = new stdClass();
       	...
       	$colaboradores = Colaboradores_orm::DeEmpresa( $empresa->id )->PorCiclo($ciclo_id)->orderBy('nombre', "ASC")->get();
     	...
     	if(!empty($rows->toArray())){
     		foreach ($rows->toArray() AS $i => $row){
     			...
     			if ($this->auth->has_permission('acceso', 'planilla/ver/(:any)')){
     				...
     				{
     					$hidden_options .= '<a href="'. base_url('planilla/ver-reporte-decimo-cerrada/'.$uuid_colaborador."~".$planilla_id) .'" data-id="'. $row['id'] .'" class="btn btn-block btn-outline btn-success">Ver detalle</a>';
     				}else {
      ...
      public function ajax_listar_ingresos_decimo() {
      	...
      	if(!empty($tabla)){
      		foreach ($tabla AS $row){
      			$response->rows[$i]["cell"] = array(
      					$row['fecha_pago'],
      					"$ ".number_format($row['ingreso'],2)
       			);
      			$total_calculo += $row['ingreso'];
      			++$i;
      		}
     	...
     	if(!empty($rows->toArray())){
 	        ...
 	        $estado = '<span style="color:white; background-color:'.$row['estado']['color_estado'].'" class="btn btn-xs btn-block">'.$row['estado']['etiqueta'].'</span>';
     			...
     			if ($this->auth->has_permission('acceso', 'planilla/ver/(:any)')){
     				...
     				$link_detalles = '<a  style="color:blue; text-decoration: underline;" href="'. base_url('planilla/ver/'. $uuid_planilla) .'">'.$row['identificador'].$row['semana'].$row['ano'].$row['secuencial'].'</a>';
     			}
     			else{
     				$link_detalles =  $row['identificador'].$row['semana'].$row['ano'].$row['secuencial'];
     			}
                ...
                if( $row['estado']['valor'] == 'validada'){
     						       $hidden_options .= '<a id="cerrarPlanilla" href="#" data-id="'. $row['id'] .'" class="btn btn-block btn-outline btn-success">Cerrar</a>';
       					}
      ...
      public function ajax_crear_planillaNoRegulares() {
     	...
     	try {
     		...
     		if($campos['tipo_creacion'] == 'liquidaciones'){
     			...
     			$deducciones = array();
     			if(!empty($campos['deducciones']["deducciones"]))
     			{
     				if(Util::is_array_empty($campos['deducciones']["deducciones"]) == false){
     					//Recorrer los dependientes
     					$j=0;
     					foreach ($campos["deducciones"]["deducciones"] AS $deduccion){
     						//$fieldset = array();
     			...
     			$acumulados = array();
     			if(!empty($campos['acumulados']["acumulados"]))
     			{
     				if(Util::is_array_empty($campos['acumulados']["acumulados"]) == false){
     					//Recorrer los dependientes
     					$j=0;
     					foreach ($campos["acumulados"]["acumulados"] AS $acumulado){
     						//$fieldset = array();
     						$fieldset["fecha_creacion"] = date('Y-m-d H:i:s');
     ...
     public function ajax_crear_planilla() {
     	...
     	try {
     		$campos = $_POST;
       		...
       		$info_tipo  = Catalogos_orm::find($campos['tipo_id']);

      		$fecha1 = explode("/", trim($campos['rango_fecha1']));
     		$rango_1 = $fecha1[2].'-'.$fecha1[1].'-'.$fecha1[0];

     		$fecha2 = explode("/", trim($campos['rango_fecha2']));
     		...
     		$fieldset = array(
     				"codigo" 	 =>  'PL'.(int)date("W", strtotime($fecha_rango_1)).(int)date("y", strtotime($fecha_rango_1)).$this->ajax_cargar_numero_secuencial(),
     				"identificador" 	 =>  'PL',
     				...
     				"pasivo_id" 		 =>  $campos['pasivo_id'],
     				"fecha_creacion" 	 =>  date("Y-m-d H:m:s"),
     				"ciclo_id" 		 	 =>  $campos['ciclo_id'],
     				"tipo_id" 		 	 =>  $campos['tipo_id']
     		);

     		if($campos['tipo_creacion'] == 'colaborador'){
      			$fieldset['centro_contable_id'] 	= 0;  //Centro
     			$fieldset['sub_centro_contable_id'] = 0;  //SubCentro
     			$fieldset['area_negocio'] 			= 0;  //Area de Negocio
     			$fieldset['nombre'] 				= 'Por Colaborador'; //Centro Contable

     			$fieldset['secuencial'] 		= $this->ajax_cargar_numero_secuencial();
     			$fieldset["uuid_planilla"] = Capsule::raw("ORDER_UUID(uuid())");
     			$planilla = Planilla_orm::create($fieldset);

     			// $planilla->colaboradores()->attach($campos['to']);

     			if(!empty($campos['to'])){
     				foreach($campos['to'] as $id){
      					$tipo =  Colaboradores_orm::where('id','=',  $id )
     					...
     					//Esta condicion se puso, en caso de decimos todos deben estar aprobados al crear
     					if($info_tipo->valor == 'xiii_mes')
     					{
     						$fieldset['estado_ingreso_horas'] 	= 3;
     					}


     					$planilla_colaborador[] 	 = new Planilla_colaborador_orm($fieldset);
     				}
     				   $planilla->colaboradores_planilla()->saveMany($planilla_colaborador);
     			}
     		...
     		else if($campos['tipo_creacion'] == 'centro'){

      			//$id_centro_contable =  $campos['centro_contable_id'];
     			$id_centro_contable =  $campos['CentrosContablesID'];

     			if(count($id_centro_contable)>0) //Validando que se seleccion� el centro contable
     			{
     				foreach($id_centro_contable as $id) //Ciclo para crear la(s) planilla(s)
     				{
     					$id_centro = explode('-',$id); //Se divide el Centro, subcentro y �rea

      					$clause = array();
      					$clause["ciclo_id"] 			= $_POST['ciclo_id'];



     					if(count($id_centro) == 3 ){ //Si se selecciona hasta las areas de negocios(Nietos)
      						 $fieldset['centro_contable_id'] 	= $id_centro[0];  //Centro
     						 $fieldset['sub_centro_contable_id'] = $id_centro[1];  //SubCentro
     						 $fieldset['area_negocio'] 			= $id_centro[2];  //Area de Negocio
     						//Funcion que llama a la lista de colaboradores de ese area de Negocios

     						$clause["empresa_id"] 			= $this->empresa_id;
     						$clause["centro_contable_id"] 	=  $id_centro[1]; //Subcentroo
     						$clause["departamento_id"] 		= array($id_centro[2]);    // o area de Negocio
     						$colaboradores = Planilla_colaborador_orm($clause); //Retorna el Array de Colaboradores que pertenecen a las areas de negocios
     					}
     					//Esta Parte es cuando se selecciona  el subcentro (Padre) //SINTERMINAR
     					else if(count($id_centro) == 2 ){ //Si solo existe centro
      						$clause["empresa_id"] 			= $this->empresa_id;
     						$clause["centro_contable_id"] 	= $id_centro[1] ; //Subcentroo
     					...
     					//Esta Parte es cuando se selecciona el centro Abuelo
     					else if(count($id_centro) == 1 ){ //Si solo existe centro (Abuelo)
     						 $fieldset['centro_contable_id'] 	= $id_centro[0];  //Centro
     						 $fieldset['sub_centro_contable_id'] = 0;  //SubCentro
     						 $fieldset['area_negocio'] 			= 0;  //Area de Negocio
     						...
     						$lista_subcentros =  Centros_orm::where('padre_id','=', $id_centro[0] )
     						->get(array('id','nombre')); //Me retorna todos los subcentros seleccioandos

     						if(!empty($lista_subcentros)){
     							foreach($lista_subcentros as $centro){
     								$subcentros[] = $centro->id; //Creo un array limpio de esos subcentros
     							}
     						}

     						$ids_colaboradores =  Colaboradores_orm::whereIn('centro_contable_id',  $subcentros )
     						->where("empresa_id","=",$this->empresa_id) //Obtengo la lista de colaboradores solo de esos subcentros
     						->where("ciclo_id","=",$_POST['ciclo_id']) //Obtengo la lista de colaboradores solo de esos subcentros
     						->get(array('id', 'tipo_salario'));
     						if(!empty($ids_colaboradores->toArray())){
     							$i= 0;
     							foreach($ids_colaboradores->toArray() as $id_colaborador){

      								$colaboradores[$i]['id'] = $id_colaborador['id'];
     								$colaboradores[$i]['tipo_salario']  = $id_colaborador['tipo_salario'];
     								++$i;
     							}
      						}
     					}

     					//Aqui sigue las Multiples inserciones en la base de datos
     					$fieldset['secuencial'] 		= $this->ajax_cargar_numero_secuencial();
     					$fieldset["uuid_planilla"] = Capsule::raw("ORDER_UUID(uuid())");
      					...
      					$planilla = Planilla_orm::create($fieldset);
       					...
       					if(!empty($colaboradores)){
       						$planilla_colaborador = array();
     						foreach($colaboradores as $colaborador){

     							$fieldsetCOl = array();
     							$fieldsetCOl['colaborador_id'] 		= $colaborador['id'];

     							$fieldsetCOl['estado_ingreso_horas'] 	= 0;

     							if($colaborador['tipo_salario'] == 'Mensual')
     								$fieldsetCOl['estado_ingreso_horas'] 	= 3;

     							$planilla_colaborador[] 	 = new Planilla_colaborador_orm($fieldsetCOl);
     						}
     						$planilla->colaboradores_planilla()->saveMany($planilla_colaborador);
     					}
     ...
     public function ajax_editar_planilla() {
     	...
     	try {
     		...
     		//$fieldsetPlanilla["ciclo_id"] 	  = $ciclo_id;
            $plant_data = $this->planillaRepository->find($planilla_id);
            $plant_data->rango_fecha1 = $fecha_rango_1;
            $plant_data->rango_fecha2 = $fecha_rango_2;
            $plant_data->pasivo_id = $pasivo_id;
     		$plant_data->save();
     		//$planillas = Planilla_orm::where('id', '=', $planilla_id)->update($fieldsetPlanilla);


     		$responseDed = Planilla_deducciones_orm::where('planilla_id', $planilla_id)->delete();
     			...
     			if(!empty($deducciones['deducciones']))
     			{
     				if(Util::is_array_empty($deducciones['deducciones']) == false){
     					...
     					foreach ($deducciones['deducciones'] AS $deduccion){
     						...
     						$deduccion = Planilla_deducciones_orm::create($fieldset);
     						$j++;
     					}
     				}
     			}
     		}
     		$responseAc = Planilla_acumulados_orm::where('planilla_id', $planilla_id)->delete();
     		if(!empty($acumulados['acumulados'])){

     			// -------- Parte de Acumualados
     			//	$acumuladosLista = array();
     			if(!empty($acumulados['acumulados']))
     			{
     				if(Util::is_array_empty($acumulados['acumulados']) == false){
     					...
     					foreach ($acumulados['acumulados'] AS $acumulado){
     						...
     						$acumulado = Planilla_acumulados_orm::create($fieldset);
      						$j++;
     					}
     				}
     			}
       	...
       	if(!empty($rows->toArray())){
     		foreach ($rows->toArray() AS $i => $row){
     			...
     			if(!empty($row['cuenta_gasto'])){
     				$cuenta_gasto_nombre = $row['cuenta_costo']['codigo'].' - '.$row['cuenta_gasto']['nombre'];
     			}

          if(!empty($row['cuenta_costo'])){
     				$cuenta_costo_nombre = $row['cuenta_costo']['codigo'].' - '.$row['cuenta_gasto']['nombre'];
     			}
...
private function _createPago( $planillaInfo) {
    ...
    $planillaInfo->load('colaboradores_pagadas.colaborador');
    ...
    $planilla_info->load(['colaboradores_planilla',
        ...
        $calculos_globales = $this->planillaRepository->reporte_colaborador($planilla_info);
 		...
 		try {
           ...
           $planilla_creada = $this->pagadasRepository->crear($calculos_globales);
...
/** ***		"liquidaciones.colaborador.salarios_devengados_ultimos_cinco_anos" => function ($query) use ($id_liquidacion)  {
	...
	public function ver_reporte_decimo($colaborador_uuid = NULL) {
		...
		if($colaborador_uuid!=NULL)
		{
			...
			//Para sacar las deducciones cuando es decimo
			$lista_deducciones = Planilla_deducciones_orm::lista_deducciones_planilla($planilla_id);
 			$tabla_deducciones= $this->calculos_deducciones_colaborador($datos_generales, $lista_deducciones);



			$lista_acumulados = Planilla_acumulados_orm::lista_acumulados_planilla($planilla_id);
			$tabla_acumulados = $this->calculos_acumulados_colaborador($datos_generales, $lista_acumulados);

			$tablas_principales  = array(
					'tabla_ingresos' => $tabla_ingresos,
					'tabla_deducciones' => !empty($tabla_deducciones[0]['deducciones'])?$tabla_deducciones[0]['deducciones']: array(),
					'tabla_acumulados' => !empty($tabla_acumulados[0]['acumulados'])?$tabla_acumulados[0]['acumulados']: array(),
					'tabla_descuentos_directos' => !empty($tabla_deducciones['Descuentos directos'])?$tabla_deducciones['Descuentos directos']['descuentos_directos']: array(),
					'salario_mensual_promedio' =>  isset($datos_generales[0]['salario_mensual_promedio_doce_meses'])?$datos_generales[0]['salario_mensual_promedio_doce_meses']:0,
					'salario_anual_promedio' =>  isset($datos_generales[0]['salario_bruto_doce_meses'])?$datos_generales[0]['salario_bruto_doce_meses']:0,
					'total_devengado' =>  isset($datos_generales[0]['total_devengado'])?$datos_generales[0]['total_devengado']:0,
					'indemnizacion_proporcional' =>  isset($datos_generales[0]['indemnizacion_proporcional'])?$datos_generales[0]['indemnizacion_proporcional']:0
			);
  ...
  /*public function ajax_detalles_pago() {
  ...
  $salarios_brutos = $this->calculos_salarios_brutos($planilla_id, $cantidad_semanas, NULL, $fecha_planilla);

    $lista_deducciones = Planilla_deducciones_orm::lista_deducciones_planilla($planilla_id);

  $descuento_flag = false;
    if(!empty($lista_deducciones)){
    foreach($lista_deducciones as $deducc){
      if(array_search("Descuentos directos",$deducc)){
        $descuento_flag = true;
      }
    }
  }

  //($planilla_id, $cantidad_semanas, $colaborador_info[0]['id'], $fecha_planilla);
  $salario_bruto_planilla = $descuentos_bruto_planilla = $bonificaciones_planilla = $descuento_directo_planilla = 0;
  if(!empty($salarios_brutos)){
    foreach($salarios_brutos as $colaborador){
      if(!empty($colaborador['lista_ingresos'])){
        foreach($colaborador['lista_ingresos'] as $calculos){
          $salario_bruto_planilla +=$calculos['calculo'];

        }
      }
      if($descuento_flag == 1){
        if(!empty($colaborador['descuentos_directos'])){
          foreach($colaborador['descuentos_directos'] as $descuento){
            $descuentos_bruto_planilla +=$descuento['monto_ciclo'];
          }
        }
      }

      $bonificaciones_planilla += $colaborador['bonificaciones'];
       $descuentos_directos = Planilla_deducciones_orm::lista_descuentos_colaborador($colaborador['colaborador_id']);
      foreach($descuentos_directos as $descuento_directo){

          $descuento_directo_planilla += $descuento_directo['monto_ciclo'];
        }
    }
  }
  ...
  //Funcion que se encargar� de calcular las deducciones por cada colaborador
  $calculo_deducciones_por_colaborador = $this->calculos_deducciones_colaborador($salarios_brutos, $lista_deducciones);


    $deducciones_bruto_planilla = 0;
  if(!empty($calculo_deducciones_por_colaborador)){
    foreach($calculo_deducciones_por_colaborador as $colaborador){
      if(!empty($colaborador['deducciones'])){
        foreach($colaborador['deducciones'] as $deduccion){
          $deducciones_bruto_planilla +=$deduccion['descuento']; //Todos los descuentos
        }
      }
    }
  }

  //$descuentos_planilla = $descuentos_bruto_planilla + $deducciones_bruto_planilla;
   $descuentos_planilla = $descuentos_bruto_planilla + $deducciones_bruto_planilla + $descuento_directo_planilla;


  $response->response = true;
  $response->cantidad_colaboradores = count($salarios_brutos);
  $response->salario_bruto 			    = number_format($salario_bruto_planilla,2);
  $response->salario_neto 			    =  number_format($salario_bruto_planilla-$descuentos_planilla,2);
  $response->salario_neto_porcentaje=  ($salario_bruto_planilla>0)?(number_format((($salario_bruto_planilla-$descuentos_planilla)/$salario_bruto_planilla)*100,2)):'0.00';
  $response->salario_neto_progress_bar  =  ($salario_bruto_planilla>0)?(number_format((($salario_bruto_planilla-$descuentos_planilla)/$salario_bruto_planilla)*100,2)."%"):'0%';

  $response->bonificaciones 			        =  number_format($bonificaciones_planilla,2);
  $response->bonificaciones_porcentaje    = ($salario_bruto_planilla>0)?(number_format(($bonificaciones_planilla/$salario_bruto_planilla)*100,2)):'0.00';
  $response->bonificaciones_progress_bar  = ($salario_bruto_planilla>0)?(number_format(($bonificaciones_planilla/$salario_bruto_planilla)*100,2)."%"):'0%';

  $response->descuentos 				      = number_format($descuentos_planilla,2);
  $response->descuentos_porcentaje 	  = ($salario_bruto_planilla>0)?(number_format(($descuentos_planilla/$salario_bruto_planilla)*100,2)):'0.00';
  $response->descuentos_progress_bar 	= ($salario_bruto_planilla>0)?(number_format(($descuentos_planilla/$salario_bruto_planilla)*100,2)."%"):'0%';

  echo json_encode($response);
  exit;
}
public function ver_reporte($colaborador_uuid = NULL) {
  ...
  if($colaborador_uuid!=NULL)
  {
      ...
      $colaborador_info = Colaboradores_orm::with('estado','cargo','centro_contable','departamento','forma_pago')
    ->where(Capsule::raw("HEX(uuid_colaborador)"), "=", $colaborador_uuid)
    ->get()->toArray();
    ...
    $salarios_brutos = $this->calculos_salarios_brutos($planilla_id, $cantidad_semanas, $colaborador_info[0]['id'], $fecha_planilla);
    $tabla_ingresos = isset($salarios_brutos[0]['lista_ingresos'])?$salarios_brutos[0]['lista_ingresos']: array();

    $lista_deducciones = Planilla_deducciones_orm::lista_deducciones_planilla($planilla_id);

    //Funcion que se encargar� de calcular las deducciones
    $tabla_deducciones= $this->calculos_deducciones_colaborador($salarios_brutos, $lista_deducciones);

    $tabla_deducciones = isset($tabla_deducciones[0]['deducciones'])?$tabla_deducciones[0]['deducciones']: array();

    $tabla_descuentos_directos = !empty($tabla_deducciones['Descuentos directos'])?$tabla_deducciones['Descuentos directos']['descuentos_directos']: array();
    $lista_acumulados = Planilla_acumulados_orm::lista_acumulados_planilla($planilla_id);
    $tabla_acumulados = $this->calculos_acumulados_colaborador($salarios_brutos, $lista_acumulados);
    $tabla_acumulados = isset($tabla_acumulados[0]['acumulados'])?$tabla_acumulados[0]['acumulados']: array();
    ...
    $tablas_principales  = array(
        ...
        'tabla_acumulados' => $tabla_acumulados,
        'tabla_descuentos_directos' => $tabla_descuentos_directos,
        'salario_mensual_promedio' =>  isset($salarios_brutos[0]['salario_mensual_promedio_doce_meses'])?$salarios_brutos[0]['salario_mensual_promedio_doce_meses']:0,
        'salario_anual_promedio' =>  isset($salarios_brutos[0]['salario_bruto_doce_meses'])?$salarios_brutos[0]['salario_bruto_doce_meses']:0,
        'total_devengado' =>  isset($salarios_brutos[0]['total_devengado'])?$salarios_brutos[0]['total_devengado']:0,
        'indemnizacion_proporcional' =>  isset($salarios_brutos[0]['indemnizacion_proporcional'])?$salarios_brutos[0]['indemnizacion_proporcional']:0
    );
    $this->session->set_userdata($tablas_principales);
...
public function exportar_ver_reporte_cerrada($colaborador_uuid = NULL){
    ...
    $tabla_ingresos 	= isset($pagos_info[0]['ingresos'])?$pagos_info[0]['ingresos']: array();
    $tabla_deducciones 	= isset($pagos_info[0]['deducciones'])?$pagos_info[0]['deducciones']: array();
    $tabla_acumulados 	= isset($pagos_info[0]['acumulados'])?$pagos_info[0]['acumulados']: array();
    $tabla_descuentos 	= isset($pagos_info[0]['descuentos'])?$pagos_info[0]['descuentos']: array();
    $tabla_calculos	= isset($pagos_info[0]['calculos'])?$pagos_info[0]['calculos']: array();

    $tabla_ingresos 	= $this->removeElementWithValue($tabla_ingresos, array("id","planilla_pagada_id","fecha_creacion"));
    $tabla_deducciones = $this->removeElementWithValue($tabla_deducciones, array("id","planilla_pagada_id","deduccion_id","fecha_creacion"));
    $tabla_acumulados = $this->removeElementWithValue($tabla_acumulados, array("id","planilla_pagada_id","saldo","fecha_creacion"));
    $tabla_descuentos = $this->removeElementWithValue($tabla_descuentos, array("id","planilla_pagada_id","descuento_id","fecha_creacion"));
    $tabla_calculos = $this->removeElementWithValue($tabla_calculos, array("id","planilla_pagada_id","fecha_creacion"));
    $titles = array(  'Deducciones', 'Acumulados', 'Descuentos', 'Calculos');
    ...
    $worksheet = $spreadsheet->getActiveSheet();
      ...
      $worksheet->setTitle("Ingresos");
    $worksheet->SetCellValueByColumnAndRow(0, 1, 'Detalle');
    $worksheet->SetCellValueByColumnAndRow(1, 1, 'Cantidad de horas');
    $worksheet->SetCellValueByColumnAndRow(2, 1, 'Rata');
    $worksheet->SetCellValueByColumnAndRow(3, 1, 'Calculo');
    if(!empty($tabla_ingresos)){
      $n = 2;
      $sum_ingresos = 0;
      foreach($tabla_ingresos as $ingreso){
        $worksheet->SetCellValueByColumnAndRow(0, $n, $ingreso['detalle']);
        $worksheet->SetCellValueByColumnAndRow(1, $n, $ingreso['cantidad_horas']);
        $worksheet->SetCellValueByColumnAndRow(2, $n, $ingreso['rata']);
        $worksheet->SetCellValueByColumnAndRow(3, $n, $ingreso['calculo']);

        $sum_ingresos += $ingreso['calculo'];
        ++$n;
      }

      $worksheet->SetCellValueByColumnAndRow(0, $n, "Total");
      $worksheet->SetCellValueByColumnAndRow(3, $n, number_format($sum_ingresos,2));
     ...
     foreach($titles as $titulo){
      ...
      //Write cells
      if($titulo == 'Deducciones'){

        $objWorkSheet->SetCellValueByColumnAndRow(0, 1, 'Deduccion');
        $objWorkSheet->SetCellValueByColumnAndRow(1, 1, 'Descuento');
        $objWorkSheet->SetCellValueByColumnAndRow(2, 1, 'Saldo');



         if(!empty($tabla_deducciones)){
          $m = 2;
          $sum_deducciones = $sum_deducciones_saldo =$sum_deducciones_saldo =  0;
          foreach($tabla_deducciones as $deduccion){

            $objWorkSheet->SetCellValueByColumnAndRow(0, $m, $deduccion['nombre']);
            $objWorkSheet->SetCellValueByColumnAndRow(1, $m, $deduccion['descuento']);
            $objWorkSheet->SetCellValueByColumnAndRow(2, $m, $deduccion['saldo']);
            $sum_deducciones += $deduccion['descuento'];
            $sum_deducciones_saldo += $deduccion['saldo'];
            ++$m;
          }
          $objWorkSheet->SetCellValueByColumnAndRow(0, $m, "Total");
          $objWorkSheet->SetCellValueByColumnAndRow(1, $m, $sum_deducciones);
          $objWorkSheet->SetCellValueByColumnAndRow(2, $m, $sum_deducciones_saldo);
        }

      }

      if($titulo == 'Acumulados'){

        $objWorkSheet->SetCellValueByColumnAndRow(0, 1, 'Detalle');
        $objWorkSheet->SetCellValueByColumnAndRow(1, 1, 'Acumulado');

        if(!empty($tabla_acumulados)){
          $m = 2;
          $sum_acumulados = 0;
          foreach($tabla_acumulados as $acumulado){

            $objWorkSheet->SetCellValueByColumnAndRow(0, $m, $acumulado['nombre']);
            $objWorkSheet->SetCellValueByColumnAndRow(1, $m, $acumulado['acumulado']);

            $sum_acumulados += $acumulado['acumulado'];
            ++$m;
          }
          $objWorkSheet->SetCellValueByColumnAndRow(0, $m, "Total");
          $objWorkSheet->SetCellValueByColumnAndRow(1, $m, number_format($sum_acumulados, 2));
        }
      }

      if($titulo == 'Descuentos'){

        $objWorkSheet->SetCellValueByColumnAndRow(0, 1, 'No. de descuento');
        $objWorkSheet->SetCellValueByColumnAndRow(1, 1, 'Acreedor');
        $objWorkSheet->SetCellValueByColumnAndRow(2, 1, 'Monto abonado');
        $objWorkSheet->SetCellValueByColumnAndRow(3, 1, 'Saldo restante');

        if(!empty($tabla_descuentos)){
          $m = 2;
          $sum_des_ciclo = $sum_saldo = 0;
          foreach($tabla_descuentos as $descuento){

            $objWorkSheet->SetCellValueByColumnAndRow(0, $m, $descuento['codigo']);
            $objWorkSheet->SetCellValueByColumnAndRow(1, $m, $descuento['acreedor']);
            $objWorkSheet->SetCellValueByColumnAndRow(2, $m, $descuento['monto_ciclo']);
            $objWorkSheet->SetCellValueByColumnAndRow(3, $m, $descuento['saldo_restante']);

            $sum_des_ciclo += $descuento['monto_ciclo'];
            $sum_saldo += $descuento['saldo_restante'];
            ++$m;
          }

          $objWorkSheet->SetCellValueByColumnAndRow(0, $m, "Total");
          $objWorkSheet->SetCellValueByColumnAndRow(2, $m, number_format($sum_des_ciclo, 2));
          $objWorkSheet->SetCellValueByColumnAndRow(3, $m, number_format($sum_saldo, 2));
      ...
      if($titulo == 'Calculos'){
        ...
        $objWorkSheet->SetCellValueByColumnAndRow(0, 1, 'Detalle');
        $objWorkSheet->SetCellValueByColumnAndRow(1, 1, 'Monto');

        if(!empty($tabla_calculos)){
          $m = 2;
          foreach($tabla_calculos as $calculo){

            $objWorkSheet->SetCellValueByColumnAndRow(0, $m, $calculo['detalle']);
            $objWorkSheet->SetCellValueByColumnAndRow(1, $m, $calculo['monto']);
            ++$m;
          }
        }
      }
    ...
    header('Content-Disposition: attachment; filename="detalles_'.date("ymdi").'.xlsx"');
    $objWriter->save('php://output');
    exit;
  }

  private function exportar_csv_cerrada_regular($planilla_id = NULL) {
    ...
    if(!empty($pagos_info)){
        foreach ($pagos_info AS $row)
      {
        ...
        $total_desc = $desc_ss = $desc_isr= $desc_se =$total_salario = $desc_cs = $ingreso_hr = $ingreso_nohr = 0;
        //descuentos
        $descuento_directo_planilla = 0;
        $descuentos_directos = Planilla_deducciones_orm::lista_descuentos_colaborador($row['colaborador_id']);
       foreach($descuentos_directos as $descuento_directo){
        ...
        if(!empty($row['deducciones'])){
          foreach($row['deducciones'] as $deduccion){
            ...
            $deduccion_key = Deducciones_orm::where("estado_id","=",1)->where("empresa_id","=",$this->empresa_id)->where("id","=",$deduccion['deduccion_id'])->get();
            if($deduccion_key[0]['key'] == 1){
                $desc_ss = $deduccion['descuento'];
            }
            if($deduccion_key[0]['key'] == 2){
                $desc_isr = $deduccion['descuento'];
            }
            if($deduccion_key[0]['key'] == 3){
                $desc_se = $deduccion['descuento'];
            }
            if($deduccion_key[0]['key'] == 5){
                $desc_cs = $deduccion['descuento'];
            }
        ...
        if(!empty($row['ingresos'])){
          foreach($row['ingresos'] as $ingreso){

            if($ingreso['detalle'] == 'HR'){
              $ingreso_hr= $ingreso['calculo'];
            }
            else{
              $ingreso_nohr = $ingreso['calculo'];
            }

            $total_salario += $ingreso['calculo'];
          }
        }
        ...
        if($colaborador_info[0]['tipo_salario'] == 'Hora' ){
          $rata_mensual = utf8_decode(Util::verificar_valor($colaborador_info[0]['rata_hora']));
        }else{
          $rata_mensual = utf8_decode(Util::verificar_valor($colaborador_info[0]['salario_mensual']));
        }
        $csvdata[$i]['posicion'] = Util::verificar_valor($colaborador_info[0]['cargo']['nombre']);
        $csvdata[$i]['nombre'] = $nombre. " ". $apellido;
        $csvdata[$i]["cedula"] = utf8_decode(Util::verificar_valor($colaborador_info[0]['cedula']));
        $csvdata[$i]["rata_hora"] = $rata_mensual;
        $csvdata[$i]["hr"] = number_format($ingreso_hr,2);
        $csvdata[$i]["he"] = number_format($ingreso_nohr,2);
        $csvdata[$i]["isr"] = number_format($desc_isr,2);
        $csvdata[$i]["se"] = number_format($desc_se,2);
        $csvdata[$i]["ss"] = number_format($desc_ss,2);
        $csvdata[$i]["cs"] = number_format($desc_cs,2);
        $csvdata[$i]["deducciones"] = number_format($total_desc,2);
        //$csvdata[$i]["descuento"] = 0;
        $csvdata[$i]["descuento"] = $descuento_directo_planilla;

        $csvdata[$i]["salario_bruto"] = number_format($total_salario,2);
        $csvdata[$i]["salario_neto"] = number_format($total_salario-$total_desc,2);
        $i++;
      }
    }
    //:
  ...
  public function ajax_pagar_planilla() {
        ...
        $planilla_info->total_colaboradores = count($colaboradores);
        $planilla_info->save();

				foreach ($colaboradores->toArray() AS $i => $row){

					$colaborador[] = $row['colaborador_id'];
					$tablas_devueltos[] = $this->buscando_tablas($planilla_id, $cantidad_semanas, $row['colaborador_id'], $fecha_planilla);
 			...
 			foreach ($tablas_devueltos as $tabla ){
    			...
    			if( !empty($tabla['tabla_descuentos_directos']) ){
 			    ...
 			    Planilla_orm::where('id', '=', $planilla_id)->update(array("estado_id"=> 14));
 		} catch(ValidationException $e){
  ...
  private function buscando_tablas($planilla_id = NULL, $cantidad_semanas = NULL, $colaborador_id = NULL, $fecha_planilla = array()) {

    $informacion_general = $this->calculos_salarios_brutos($planilla_id, $cantidad_semanas,$colaborador_id, $fecha_planilla);

    $lista_deducciones = Planilla_deducciones_orm::lista_deducciones_planilla($planilla_id);

    $tabla_ingresos = isset($informacion_general[0]['lista_ingresos'])?$informacion_general[0]['lista_ingresos']: array();

    //$tabla_descuentos_directos = isset($informacion_general[0]['descuentos_directos'])?$informacion_general[0]['descuentos_directos']: array();
    $tabla_descuentos_directos = array();
    $tabla_deducciones= $this->calculos_deducciones_colaborador($informacion_general, $lista_deducciones);
    $tabla_deducciones = isset($tabla_deducciones[0]['deducciones'])?$tabla_deducciones[0]['deducciones']: array();
    ...
    $colaborador_info = Colaboradores_orm::find($colaborador_id);
    $colaborador_info->load("colaboradores_contratos_activo");

    $tabla_pagadas_colaborador[] = array(
        'salario_bruto' => $informacion_general[0]['salario_bruto'],
        'planilla_id' => $planilla_id,
        'uuid_colaborador' => hex2bin($colaborador_info->uuid_colaborador),
        'contrato_id' => isset($colaborador_info->colaboradores_contratos_activo[0])?$colaborador_info->colaboradores_contratos_activo[0]->id:0,
        'colaborador_id' => $colaborador_id,
        'fecha_cierre_planilla' => date("Y-m-d H.i:s"),
        'fecha_creacion' => date("Y-m-d H.i:s")
    );
...
*/
/*private function calculo_para_liquidaciones($planilla_id = NULL, $cantidad_semanas= NULL,$tipo= NULL) {

  $bruto_total = $deduccion_total = 0;

  $datos_generales = $this->calculos_salarios_brutos_especiales( $planilla_id, $cantidad_semanas, $tipo );

  //Lista de Acumulados de esta plantilla
  $lista_acumulados = Planilla_acumulados_orm::lista_acumulados_planilla($planilla_id);
  $tabla_acumulados = $this->calculos_acumulados_colaborador($datos_generales, $lista_acumulados);
  if(!empty($tabla_acumulados)){
    foreach($tabla_acumulados as $acumulado){
      //if(!empty($acumulado['acumulados'])){
      //$vacacion_planilla += $acumulado['acumulados'][0]['acumulado'];
      if(!empty($acumulado['acumulados'])){
        foreach ($acumulado['acumulados'] as $calculo){
          $bruto_total +=  $calculo['acumulado'];
        }
      }
    }
  }

  //Lista de deducciones de esta plantilla
  $lista_deducciones = Planilla_deducciones_orm::lista_deducciones_planilla($planilla_id);
