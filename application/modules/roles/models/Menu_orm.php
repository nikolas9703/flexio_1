...
class Menu_orm extends Model {
    ...
    public static function lista_menu_superior() {
        $results = Capsule::table('modulos')
                ...
                ->get(array('agrupador', 'controlador', 'menu', 'agrupador_orden','icono'));
     ...
     */
    public static function lista_menu_lateralPermiso($agrupador = NULL) {
        if ($agrupador == NULL) {
            return false;
        }
        $a = array();
        $results = Capsule::table('modulos')
                ...
                ->distinct()
                ->get(array('id', 'icono', 'agrupador', 'nombre', 'grupo', 'menu', 'controlador'));
        ...
        if (!empty($results)) {
            foreach ($results AS $result) {
                ...
                if (!empty($agrupadores) && is_array($agrupadores)) {
                    if (!empty($agrupadores["nombre"][0]) && is_array($agrupadores["nombre"][0])) {
                        foreach ($agrupadores["nombre"][0] AS $agrupadornombre => $info) {
                            if (preg_match("/($agrupador)/i", strtolower($agrupadornombre)))
                                $grupo_orden = $info["grupo_orden"];
                        }
                    }else {
                        foreach ($agrupadores["nombre"] AS $agrupadornombre => $array) {
                            if (preg_match("/($agrupador)/i", strtolower($agrupadornombre)))
                                $grupo_orden = $array["grupo_orden"];
                        }
                    }
                }
                ...
                $menu[$grupo]["id"] = $idM;                
                $menu[$grupo]["icono"] = $icono;
            }
        }
        return $menu;
    }

    public static function listarPermisos($controllador, $modulo_id) {
        $results = Capsule::table('modulos as m')
                ->join("recursos as r", "m.id", "=", "r.modulo_id")
                ->where('r.modulo_id', $modulo_id)
                ->where('r.nombre', 'LIKE', "%$controllador%")
                ->get(array('r.nombre as nombre'));
        $results2 = Capsule::table('modulos as m')
                ->join("recursos as re", "m.id", "=", "re.modulo_id")
                ->join("permisos as p", "p.recurso_id", "=", "re.id")
                ->join("roles_permisos as r", "r.permiso_id", "=", "p.id")
                ->where('re.modulo_id', $modulo_id)
                ->where('re.nombre', 'LIKE', "%$controllador%")
                ->get(array('re.nombre as nombre'));
        $arrayPermisos = [];
        foreach ($results AS $result) {
            $permiso = explode("/", $result->nombre);
            if (isset($permiso[1]))
                $arrayPermisos[$permiso[1]] = 0;
        }
        foreach ($results2 AS $result) {
            $permiso = explode("/", $result->nombre);
            if (isset($permiso[1]))
                $arrayPermisos[$permiso[1]] = 1;
        }
