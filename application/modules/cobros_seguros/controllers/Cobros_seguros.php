...
use Flexio\Modulo\Cobros_seguros\Models\CobroAnular;
...
class Cobros_seguros extends CRM_Controller {
  ...
  protected $ClienteRepository;
	...
	function ver($uuid=NULL) {
		...
		$acceso_editar = 1;
		if(!$this->auth->has_permission('acceso','cobros_seguros/editar-agendado')){
			$acceso_editar = 0;
		}
		...
		$politica = 0;
		$Roles = RolesUsuario::where(['empresa_id' => $this->empresa_id, 'usuario_id' => $this->usuario_id])->get();
		$PoliticasArray = array();
		foreach ($Roles as $key => $value) {
			
			$politicas = Politicas::where(['role_id' => $value['role_id'], 'empresa_id' => $this->empresa_id, 'modulo' => 'cobros', 'estado_id' => 1])->get();
			if(count($politicas) > 0){
				foreach ($politicas as $key => $value) {
					var_dump($value->politica_estado);
					$transaccionPolitica = PoliticasCatalogo::where(['id' => $value->politica_estado, 'tipo' => 'cobros'])->first();
		...
		$this->assets->agregar_var_js(array(
			...
			"regreso" =>$regreso,
			'ids_polizas' => '',
			'accedo_editar' => $acceso_editar,
			'estado' => $cobro->estado,
			'politicas' => json_encode($PoliticasArray)
		));
		//$this->desde_empezables();
		$data['mensaje'] = $mensaje;
		$breadcrumb = array(
   ...
   */
    ...
    public function ajax_get_numero(){
    	$id_cobro = $this->input->post('id_cobro');

    	$datosCobro = Cobros_seg::where(['id' => $id_cobro])->first();
    	$this->output->set_status_header(200)->set_content_type('application/json', 'utf-8')->set_output($datosCobro)->_display();
		exit;
    }

    public function ajax_set_cobroAnulado(){
    	$id_cobro = $this->input->post('id_cobro');
    	$motivo = $this->input->post('motivo');
    	$otro_motivo = $this->input->post('otro_motivo');
    	$inf = array();
    	try {
    		$datosAnular['cobro_id'] = $id_cobro;
    		$datosAnular['motivo'] = $motivo;
    		$datosAnular['otros'] =$otro_motivo;
    		$inf["msg"] = CobroAnular::create($datosAnular);
