...
class Pagos extends CRM_Controller
{
        ...
        if ($count) {
            foreach ($pagos->get() as $i => $row) {
                ...
                if ($row->formulario != 'planilla' && $row->formulario != 'pago_extraordinario' /* && $row->formulario != 'movimiento_monetario' */) {
                  ...
                  if($row->estado == 'por_aplicar' &&   trim($this->listaPago->metodo_pago($row->metodo_pago)) == 'Cheque'){
                      $hidden_options .= 'Se debe imprimir el cheque.';
                  }
                  ...
                  if ($row->estado == 'por_aprobar') {
                          $hidden_options .= '<a href="#" data-id="'.$row->uuid_pago.'" data-pagoid="'.$row->id.'"  class="btn btn-block btn-outline btn-success aprobarPagoPE" >Aprobar pago</a>';
                  }
                  elseif ($row->estado == 'cheque_en_transito' and trim($this->listaPago->metodo_pago($row->metodo_pago)) == 'Cheque') {
                        $hidden_options .= '<a href="#" data-id="'.$row->uuid_pago.'" data-pagoid="'.$row->id.'" class="btn btn-block btn-outline btn-success aplicarPago" id="#aplicarPago">Aplicar pago</a>';
                  }
    ...
    public function ver($uuid = null)
    {
        ...
        //assets
		$prueba=$this->pagosRep->getColletionPago($registro);
    ...
    public function ajax_get_empezables()
    {
        ...
        if(!empty($request['empezable_type'])){
			...
			elseif ($request['empezable_type'] == 'participacion') {
                $result = \Flexio\Modulo\HonorariosSeguros\Models\HonorariosSeguros::where('empresa_id',17)->take(10)->$method($method == 'find' ? $request['id'] : ['*']);
            }
			elseif ($request['empezable_type'] == 'remesas_salientes') {
                $result = \Flexio\Modulo\Remesas\Models\Remesa::where('empresa_id',17)->take(10)->$method($method == 'find' ? $request['id'] : ['*']);
            }
			...
			else if($request['empezable_type'] == 'participacion')
			{
				$response = $method == 'find' ? ['id' => $result->id, 'nombre' => $result->no_honorario] : $result->map(function($row){
                    return ['id' => $row->id, 'text' => $row->no_honorario];
                });
			}
			else if($request['empezable_type'] == 'remesas_salientes')
			{
				$response = $method == 'find' ? ['id' => $result->id, 'nombre' => $result->remesa] : $result->map(function($row){
                    return ['id' => $row->id, 'text' => $row->remesa];
                });
			}
			else{
    ...
    public function ajax_aprobar_pagoPE()
    {
        ...
        try {
            ...
            $pago = $this->pagosRep->find($id);

            //Si a un pago de tipo cheque se cambia de estado a un estado por aplicar este debe Generar un cheque estado por Imprimir
            if (count($pago->metodo_pago)>0 && isset($pago->metodo_pago[0]->tipo_pago) && $pago->metodo_pago[0]->tipo_pago == 'cheque') {
                $array_cheque = [];
...
//Solo para pagas que no sean de tipo Planilla, esta funcion ya no se usa
