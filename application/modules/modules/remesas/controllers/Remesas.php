...
public function guardar(){
    ...
    try {*/
        ...
        $datosCobros = array_unique($cobros);
        foreach($datosCobros as $value){
...
public function ajax_cambiar_estado_remesa() {
    ...
    try {
        if($campos['estado'] == "Anulado"){
            $datosPagos = Pagos::where(['empezable_id' => $ids])->first();
            if($datosPagos->estado != "aplicado"){
            ...
            $reclamo = Remesa::where('empresa_id', $empresa_id)->whereIn('id',$ids)->update($campo);
            $remesas = Remesa::where(['id' => $ids])->first();
            $this->crear_pago_seguros($remesas,$estado_anterior); 
        }
...
public function crear_pago_seguros($datosRemesas = null, $estado_anterior_remesa = null){
    ...
    if(isset($datosRemesas) && !empty($datosRemesas)){
        if( ($estado_anterior_remesa == "En Proceso" && $datosRemesas->estado == "Por Pagar") || ($estado_anterior_remesa == '' && $datosRemesas->estado == "Por Pagar") ){
        ...
        }elseif($estado_anterior_remesa == "Por Pagar" && $datosRemesas->estado == "En Proceso"){
