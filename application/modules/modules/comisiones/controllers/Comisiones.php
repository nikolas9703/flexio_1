...
class Comisiones extends CRM_Controller
{
    ...
    public function ajax_guardar_documento( )
    {
        ...
        $comision_id = $this->input->post('comision_id', true);
        $modeloInstancia = Comisiones_orm::where('id','=',$comision_id)->first();
    ...
    private function _createPago( $comisionInfo) {
        ...
        $contador = 1;
         ...
         foreach ($comisionInfo->colaboradores as $key => $value) {
           ...
           if($value->monto_neto > 0 &&  count($value->colaborador) > 0 ){
             $aux = [];
             $pago = new Pagos_orm;
             ...
             if($comisionInfo->metodo_pago == 13){ //ACH
             ...
             if($comisionInfo->metodo_pago == 14){ //Cheque
               $referencia = $this->pagoGuardar->tipo_pago('cheque', array(
                 'numero_cheque'=>'',
                 'nombre_banco_cheque'=>''
               ));
               $item_pago->tipo_pago = 'cheque';
             }else if($comisionInfo->metodo_pago == 13){ //ach
                if($value->colaborador->forma_pago_id == 151){ //Cheque
                  $referencia = $this->pagoGuardar->tipo_pago('cheque', array(
                    'numero_cheque'=>0,
                    'nombre_banco_cheque'=>''
                  ));
