...
<div class="row hidden" id="tabla_remesas_procesadas"> <!-- style="margin-top:-50px;"  -->
    ...
    <table class="table" id="facturaItems" >
        <thead>
            <tr>
			<th width="1%"><input type='checkbox' id='selectAllPorLiquidar'></th>
        ...
        <tbody>
			<tr v-for="remesas in informacionRemesas" id="items{{$index}}" class="item-listing" style="background-color:#ffffff">
					...
					<template v-if="remesas.id!==''">
						<input type='checkbox' class='checkboxcompletos' id='check_{{remesas.id}}' value='{{remesas.id}}' name='factura_check_id[]' />
					</template>
					</td>
					...
					<td style="{{remesas.estilos}}" id='pagada_{{remesas.id}}'>
						...
						<template v-else>
							...
							<template v-else>
								...
								<input type='hidden' id='com_final_comision' name='com_final_comision' value='{{remesas.com_paga_final}}'>
								<input type='hidden' id='no_recibo_guardar' name='no_recibo_guardar' >
	...
	<div class="form-group col-xs-12 col-sm-3 col-md-2 col-lg-2 hidden">
