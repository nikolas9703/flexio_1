...
use Flexio\Modulo\Agentes\Models\Agentes as Agentes;
use Flexio\Modulo\aseguradoras\Models\Aseguradoras as Aseguradoras;
...
class Proveedores extends CRM_Controller
{
    ...
    public function ajax_get_proveedores()
    {
		...
		if($request['tipo']=='participacion')
		{
			$method = (isset($request['id']) && !empty($request['id'])) ? 'find' : 'get';
			$result = Agentes::where('id_empresa',$this->id_empresa)->take(10)->$method($method == 'find' ? $request['id'] : ['*']);
			$response = $method == 'find' ? ['id' => $result->id, 'nombre' => $result->nombre, 'retiene_impuesto' => '' ] : $result->map(function($row){
				return ['id' => $row->id, 'text' => $row->nombre, 'retiene_impuesto' =>''];
			});
		}
		else if($request['tipo']=='remesas_salientes')
		{
			$method = (isset($request['id']) && !empty($request['id'])) ? 'find' : 'get';
			$result = Aseguradoras::where('empresa_id',$this->id_empresa)->take(10)->$method($method == 'find' ? $request['id'] : ['*']);
			$response = $method == 'find' ? ['id' => $result->id, 'nombre' => $result->nombre, 'retiene_impuesto' => '' ] : $result->map(function($row){
				return ['id' => $row->id, 'text' => $row->nombre, 'retiene_impuesto' =>''];
			});
		}
    ...
    public function ajax_get_proveedor_pago()
    {
		...
		{
			$proveedor = Agentes::find($proveedor_id);
			$registro = array();
			...
			if (count($proveedor)) {
				...
				$registro['saldo'] = 0;
				$registro['credito'] = 0;
				$registro['forma_pago'] = 0;
				$registro['banco_id'] = '';
				$registro['numero_cuenta'] = '';
				$registro['termino_pago'] = '';
			}
		}
		else if($tipo=='remesas_salientes')
		{
			$proveedor = Aseguradoras::find($proveedor_id);
			$registro = array();
