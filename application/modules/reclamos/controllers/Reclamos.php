...
class Reclamos extends CRM_Controller {
    ...
    public function editar($uuid_reclamos = null) {
        ...
        $reclamoInfoDed = ReclamosDeduccion::where("id_reclamo", $reclamos->id)->select("id_poliza_deduccion")->get();
        $ded = array();
            ...
            try {
                if (empty($campo['uuid'])) {
                    ...
                    if (isset($_POST['campodeducciones']) && $_POST['campodeducciones'] != "") {
                        ...
                        foreach ($deducciones1 as $value) {
                            $cam = array("deduccion" => $value['deduccion'], "valor_deduccion" => $value['valor_deduccion'], "id_reclamo" => $idreclamo);
...
//                    if (in_array(21, $politicas) || in_array(22, $politicas)) {
    ...
    function ajax_get_reclamantes() {
        ...
        if ($tipo_interes=="5") {
            //Personas
            $int = PolizasPersonas::where("id_poliza", $pol)->orderBy("numero","ASC")->select("numero")->get();
            $i=1;
            foreach ($int as $value) {
                $ar = array("nombre"=>$value->numero);
    ...
    function ajax_get_coberturas() {
        ...
        $polizacob = new Flexio\Modulo\Polizas\Models\PolizasCobertura;
        $coberturas = $polizacob->where("id_poliza", $pol)->get()->toArray();
        $polizaded = new Flexio\Modulo\Polizas\Models\PolizasDeduccion;
        $deducciones = $polizaded->where("id_poliza", $pol)->get()->toArray();
