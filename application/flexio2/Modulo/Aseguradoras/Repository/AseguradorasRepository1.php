<?php
namespace Flexio\Modulo\SegurosAseguradoras\Repository;

use Flexio\Modulo\SegurosAseguradoras\Models\SegurosAseguradoras;
use Flexio\Modulo\Usuarios\Models\Usuarios;
use Flexio\Modulo\Cliente\Models\Cliente;

class SegurosAseguradorasRepository {
    
   public function listar($clause=array(), $sidx=NULL, $sord=NULL, $limit=NULL, $start=NULL) {	
	//filtros
	
	var_dump("holaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");exit();
        $aseguradoras = SegurosAseguradoras::deEmpresa($clause["empresa_id"]); 
       
		//Si existen variables de orden
        if($sidx != 'nombre'){
        $aseguradoras->orderBy('nombre', 'ASC');
        }
