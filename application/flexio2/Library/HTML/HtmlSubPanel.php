...
class HtmlSubPanel{
    ...
    public static function generarHtml($subpanels,$id_modulo_data,$modulo){
        ...
        foreach ($subpanels as $key => $subpanel) {
            ...
            $html .= ''
                    . ' <li class="' . $activo . '" role="presentation"' . self::getHtmlAttr($subpanel) . ' >
                            <a href="#tabla' . ucfirst($aux) . '" data-toggle="tab" aria-controls="tabla' . ucfirst($aux) . '" role="tab" data-toggle="tab">'
        ...
        foreach ($subpanels as $key => $subpanel) {
            $activo = $i == 0 ? 'active' : '';
            $aux = isset($subpanel["html_id"]) ? $subpanel["html_id"] : $subpanel["modulo"];
            $html .= '<div role="tabpanel" class="tab-pane ' . $activo . '" id="tabla' . ucfirst($aux) . '" ' . self::getHtmlAttr($subpanel) . '>' . $modulo::run($subpanel["modulo"] . '/'.$subpanel["view"], $id_modulo_data) . '</div>';
            $i++;
        }
        $html .= '</div></div>';
        return $html .= '</div>';
