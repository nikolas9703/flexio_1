...
class TransaccionFactura implements InterfaceTransaccion{
  ...
  function iteams_transacciones($modelo){
    ...
    //datos de la factura
    $asientos[] = new Transaccion_orm (array('codigo'=>$modelo->codigo,'nombre'=>$modelo->codigo. ' '. $modelo->cliente->nombre,'debito'=>$modelo->total,'cuenta_id'=>$cuenta_id,'empresa_id'=>$modelo->empresa_id));
      if($modelo->formulario == 'refactura'){
        $asientos[] = new Transaccion_orm (array('codigo'=>$modelo->codigo,'nombre'=>$modelo->codigo. ' '. $modelo->cliente['nombre'],'credito'=>$modelo->total,'cuenta_id'=>$modelo->cuenta,'empresa_id'=>$modelo->empresa_id));
      }
      //datos de los items
