	...
	public function calcular_tarifa(){
		...
		foreach($this->tarifas AS $periodo => $tarifa) {
			...
			|| is_string($periodo) && preg_match("/diario/im", $periodo) && $this->dias_transcurridos()
				...
				$this->fecha_cargo = Carbon::parse(Carbon::parse($this->fecha_cargo)->copy())->{$tarifa["func_addition"]}(($cantidad_dias*$calculo_periodo));

				//echo "FECHA CARGO: ". $this->fecha_cargo ." <br>";
				//echo "FECHA DEVOLUCION: ". $this->fecha_devolucion ." <br>";
				//echo "CANTIDAD DIAS: ". $cantidad_dias ." <br>";
