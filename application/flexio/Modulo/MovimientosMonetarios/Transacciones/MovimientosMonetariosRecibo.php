...
class MovimientosMonetariosRecibo
{
    ...
    private function _debito($recibo_dinero)
    {
        ...
        $cliente = count($recibo_dinero->cliente) ? " - {$recibo_dinero->cliente->nombre}" : '';
        $proveedor = count($recibo_dinero->proveedor) ? " - {$recibo_dinero->proveedor->nombre}" : '';
        $asientos[] = new AsientoContable([
            'codigo' => $recibo_dinero->codigo,
            'nombre' => $recibo_dinero->codigo.$cliente.$proveedor,
