...
class TransaccionCobro implements InterfaceTransaccion{
	...
	function acredito($modelo){
		...
		foreach($modelo->cobros_facturas->where('transaccion',1) as $cobro_factura){
			$factura = $cobro_factura->facturas;
			$asientos[] = new AsientoContable(['codigo'=>$modelo->codigo,'nombre'=>$modelo->codigo. '-'.$factura->codigo, 'credito'=> $cobro_factura->monto_pagado,'cuenta_id'=>$cuenta_id,'empresa_id'=>$modelo->empresa_id]);
