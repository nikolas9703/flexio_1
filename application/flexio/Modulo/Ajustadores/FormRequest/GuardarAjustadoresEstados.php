<?php
namespace Flexio\Modulo\Ajustadores\FormRequest;
...
class GuardarAjustadoresEstados{
    protected $request;
    protected $session;
    ...
    function guardar(){
          ...
          $Ajustadores = Ajustadores::where(function($query)use($empresa,$ids){
              $query->where('empresa_id', $empresa)
                    ->whereIn('id',$ids);
                })->get();

            return Capsule::transaction(function() use($campo, $Ajustadores){
              return $Ajustadores->map(function($ant) use($campo){
                  $ant->update($campo);
                  return $ant;
              });
        });
