...
class ColaboradoresRepository implements ColaboradoresInterface{
    ...
    private function _getDescuentosDirectos($colaborador)
    {
        ...
        return [
            ...
            "monto_total"       => $colaborador->descuentos_directos->sum("monto_adeudado"),
            "monto_por_ciclo"   => $colaborador->descuentos_directos->sum("monto_ciclo"),
            "pendiente"         => $this->_getPendienteDescuentosDirectos($colaborador->descuentos_directos)
        ];
    }
    ...
    private function _getPendienteDescuentosDirectos($descuentos_directos)
    {
        ...
        {
            $fechaActivo    = !empty($dd->updated_cat) ? Carbon::createFromFormat("Y-m-d H:i:s", $dd->updated_at) : Carbon::now();
