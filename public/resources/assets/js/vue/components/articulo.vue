...
<template>
    ...
    <tr :style="(row.facturado || configv.vista == 'crear') ? 'background:white;' : 'background:orange;'" class="animated"
    transition="listado">
        ...
        <td class="item{{parent_index}} ">
            <input type="hidden" name="items[{{parent_index}}][item_hidden]" class="item_hidden" id="item_hidden{{parent_index}}" v-model="row.item_hidden">
     ...
     <td class="cantidad{{parent_index}}" > <!--:class="mostrar.icono_pop_over?'input-group':''" -->
            <input type="text" name="items[{{parent_index}}][cantidad]" class="form-control cantidad valid" data-rule-required="true" aria-required="true" id="cantidad{{parent_index}}" aria-required="true" v-model="row.cantidad" v-inputmask3="row.cantidad" :config="config.inputmask.numero" :disabled="config.disableArticulos || disabledArticulo">
...
export default {
   ...
   events: {
       ...
       setTarifa:function(){
           ...
           var context = this;
           var catalogo_tarifa = {por_hora:'hora','4_horas':'tarifa_4_horas','6_dias':'tarifa_6_dias','15_dias':'tarifa_15_dias','28_dias':'tarifa_28_dias','30_dias':'tarifa_28_dias',diario:'diario','semanal':'semanal',mensual:'mensual'};
