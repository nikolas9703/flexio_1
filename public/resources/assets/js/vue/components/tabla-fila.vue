...
<template>
    ...
    <tr :style="(row.facturado || config.vista == 'crear') ? 'background:white;' : 'background:orange;'" class="animated"
    transition="listado">
        ...
        <td class="categoria{{parent_index}} ">
            <select name="items[{{parent_index}}][categoria]" class="categoria" id="categoria{{parent_index}}" data-rule-requiredvalidation="true" aria-required="true" v-select2="row.categoria_id" :config="config.select2" :disabled="config.disableArticulos || disabledArticulo">
        ...
        <td class="item{{parent_index}} ">
            ...
            <input type="hidden" name="items[{{parent_index}}][item_id]" class="item_hidden pop_over_precio_id" id="item{{parent_index}}" v-model="row.item_id">
            <input  type="hidden"  id="comentario{{parent_index}}" name="items[{{parent_index}}][comentario]" value="{{row.comentario}}">
            <div class="input-group">
              <typeahead :item_url.sync="item_url" :categoria_id.sync="categoria_id" :parent_index="parent_index" :disabled="config.disableArticulos || disabledArticulo"> </typeahead>
              ...
              <span class="input-group-btn">
                  ...
                  <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-expanded="false" v-if="config.modulo == 'pedidos'">
                      <span class="caret"></span>
                  </button>
                  <ul class="dropdown-menu " v-if="config.modulo == 'pedidos'">
                      <li>
                          <quick-add :row.sync="row" :config.sync="config" :catalogos="catalogos" :parent_index="parent_index"></quick-add>
                      </li>
                  </ul>
              </span>
        ...
        <td class="atributo{{parent_index}} ">
            ...
            <select name="items[{{parent_index}}][atributo_id]" class="atributo" id="atributo_id{{parent_index}}" v-if="row.atributos.length > 0" v-select2="row.atributo_id" :config="config.select2" :disabled="config.disableArticulos || disabledArticulo">
                <option value="">Seleccione</option>
        ...
        <td class="cantidad{{parent_index}}  input-group">
            <input type="text" name="items[{{parent_index}}][cantidad]" class="form-control cantidad" data-rule-requiredvalidation="true" id="cantidad{{parent_index}}" v-model="row.cantidad" v-inputmask="row.cantidad" :config="config.inputmask.currency2" :disabled="config.disableArticulos || disabledArticulo">
        ...
        <td class="unidad{{parent_index}} ">
            ...
            <select name="items[{{parent_index}}][unidad]" class="unidad" id="unidad{{parent_index}}" data-rule-requiredvalidation="true" aria-required="true" v-select2="row.unidad_id" :config="config.select2" :disabled="config.disableArticulos || disabledArticulo">
                <option value="">Seleccione</option>
        ...
        <td class="precio_unidad{{parent_index}} " v-if="precioCompra() || precioInventario() || precioVenta()">
            <div class="input-group" v-if="precioCompra() || precioInventario()">
                ...
                <input
                    ...
                    data-rule-requiredvalidation="true"
                    aria-required="true"
            ...
            <div class="input-group" v-if="precioVenta()">
                ...
                <input
                    type="text"
                    name="precio_unidad_txt"
                    class="form-control precio_unidad valid"
                    data-rule-requiredvalidation="true"
                    aria-required="true"
                    v-model="row.precio_unidad"
                    v-show="config.editarPrecio">
                <input
                    type="text"
                    name="items[{{parent_index}}][precio_unidad]"
                    class="form-control precio_unidad valid"
        ...
        <td class="cuenta{{parent_index}}" v-if="!( precioCompra() || precioInventario() || precioVenta() )">
            <select name="items[{{parent_index}}][cuenta]" class="cuenta" id="cuenta{{parent_index}}" data-rule-requiredvalidation="true" aria-required="true" v-select2="row.cuenta_id" :config="config.select2" :disabled="config.disableArticulos || disabledArticulo">
                <option value="">Seleccione</option>
                <option :value="cuenta.id" v-for="cuenta in getCuentas">{{cuenta.codigo +' '+ cuenta.nombre}}</option>
            </select>
        </td>

        <td style="background: white;">
            <button type="button" class="btn btn-default btn-block eliminarBtn" agrupador="items" label="<i class=&quot;fa fa-trash&quot;></i>" @click="removeRow(row)" :disabled="config.disableArticulos || disabledArticulo"><i class="fa fa-trash"></i></button>
        ...
        <td colspan="7">
            <table style="width: 100%;background: #A2C0DA;">
                ...
                <td class="impuesto{{parent_index}}" width="33%" style="padding: 10px;">
                    ...
                    <select name="items[{{parent_index}}][impuesto]" class="impuesto" id="impuesto{{parent_index}}" data-rule-requiredvalidation="true" aria-required="true" v-select2="row.impuesto_id" :config="config.select2" :disabled="config.disableArticulos || disabledArticulo || precioInventario()">
                        <option value="">Seleccione</option>
                ...
                <td class="descuento{{parent_index}}" width="33%" style="padding: 10px;">
                    ...
                    <div class="input-group" style="width: 100%;">
                        <input type="input-right-addon" name="items[{{parent_index}}][descuento]" class="form-control descuento" id="descuento{{parent_index}}" agrupador="items" v-model="row.descuento" v-inputmask="row.descuento" :config="config.inputmask.descuento" :disabled="config.disableArticulos || disabledArticulo || precioInventario()">
    ...
    <tr v-show="fa_caret == 'fa-caret-down'" v-if="precioInventario() && (row.tipo_id == 5 || row.tipo_id == 8) && row.cantidad > 0">
        <td></td>
        <td colspan="7">
            <div style="width: 100%;background: #A2C0DA;padding:5px;">

                <div style="margin-top:10px !important;margin-bottom:10px !important" class="form-group col-xs-12 col-sm-6 col-md-6 col-lg-3" v-for="serial in row.seriales">
                    <input type="text" class="form-control" name="items[{{parent_index}}][seriales][{{$index}}]" aria-required="true" data-rule-required="true" v-model="serial.nombre" :disabled="config.disableArticulos">
                </div>
...
export default {
      ...
      computed: {
          ...
          getCuentas:function(){
              ...
              if(context.row.cuentas.length > 2 && context.row.cuentas != 'null'){
                  return _.filter(context.catalogos.cuentas, function(cuenta){
                      if(context.precioCompra() || context.config.modulo == 'pedidos')
                      {
                        return context.row.cuentas.indexOf("costo:"+ cuenta.id +"\"") > -1
                      }
                      else if (context.precioVenta())
                      {
                        return context.row.cuentas.indexOf("ingreso:"+ cuenta.id +"\"") > -1
                      }
                      return context.row.cuentas.indexOf(":"+ cuenta.id +"\"") > -1
                  });
              }
      ...
      methods:{
          ...
          getItemsAjax:function(categoria){
              var context = this;
              var datos = $.extend({erptkn: tkn},categoria,{'ventas':context.precioVenta() ? 1 : 0, item_id:context.row.item_id});
