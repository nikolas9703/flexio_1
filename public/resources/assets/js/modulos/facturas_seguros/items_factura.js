...
Vue.component('items_factura', {
	...
	components: {
		...
		'items_venta': require('./../../vue/components/tabla-dinamica-facturas-seguros.vue')
			...
			this.$nextTick(function () {
				...
				if (typeof infofactura.cobros != 'undefined') {
					...
					infofactura.cobros.forEach(function (cobro) {
						if (cobro.estado == "aplicado" || cobro.estado == "agendado" || cobro.estado == "por_aplicar") {
	...
	computed: {
		...
		'total': function () {
			var estado = '';
			return roundNumber(parseFloat(this.factura.subtotal) + parseFloat(this.factura.impuesto) - parseFloat(this.factura.descuento), 2);
		...
		'saldo': function () {
			...
			return "anulada" == this.factura.estado_id ? 0 : roundNumber(parseFloat(this.factura.total - this.cobros), 2);
