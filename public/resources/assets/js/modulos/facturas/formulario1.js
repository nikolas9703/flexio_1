...
var form_crear_facturas1 = new Vue({
    ...
    methods:{
        cargarCatalogos() {
			...
			var catalogos = this.postAjax('facturas/ajax_formulario_catalogos', datos);
    ...
    cargarValoresPorDefecto() {
      ...
      var self = this;
			...
			/*var response = this.postAjax('facturas/ajax_default_values', datos);
      response.then((response) => {
		...
		getFactura() {
			if (!_.isUndefined(window.hex_factura) && this.config.vista == "editar") {
				...
				var info = this.postAjax('facturas/ajax_factura_info', datos);
