$(function(){
	...
	$('#vehiculo').validate({
	 ...
	 $('input[name="campo[unidad]').rules(
		...
		$( "#chasis" ).keypress(function() {
		  setTimeout(function () {  
		  register_user();
		  }, 300);
		});

		$( "#chasis" ).blur(function() {
		  setTimeout(function () {  
		  register_user();
		  }, 300);
		});
		
		$("#chasis").keydown(function(){
...
function register_user()
{
	$.ajax({
		...
		{                   
			if(data === 'USER_EXISTS')
			{
				toastr.warning('Este N°. Chasis ya existe');
...
$(document).ready(function(){
	...
	$('#add_file_vehiculo').click(function(){
		...
		$('#file_tools_vehiculo').before('<div class="file_upload" id="f'+counter+'"><input name="nombre_documento[]" type="text" style="width: 300px!important; float: left;" class="form-control"><input name="file[]" class="form-control" style="width: 300px!important; float: left;" type="file"></div>');
