var retirosCrear = {
    ...
    init: function () {
        ...
        $("table#transaccionesTable").find('input.debito').each(function (index, element) {
        ...
        this.settings.formulario.find('.agregarTransaccionesBtn').tablaDinamica({
            ...
            afterAddRow: function (row) {
                ...
                retirosCrear.actualizar_chosen();
                //$('input.debito').bind('input',retirosCrear.calcular_total_debito);
                $('input.debito').bind('input', retirosCrear.calcular_total_debito);
                if ($('.chekbox-incluir').is(':checked')) {
                    var nombre = $('#retirosForm').find('input[id*="campo[nombre]"]').val();
                    if (nombre !== "") {
                        $.each($('#retirosForm').find('#transaccionesTable input[id*="nombre"]'), function (i, campo) {
                            $(this).val(nombre);
                        });
                    }
                } else {
                ...
                $(row).find('input.debito').each(function (index, element) {
            ...
            afterDeleteRow: function (row) {
                ...
                retirosCrear.calcular_total_debito();
            }
        ...
        this.settings.formulario.validate({
            ...
            errorPlacement: function (error, element) {
                ...
                if ($('#transaccionesTable').find('input[id*="nombre"]').length > 0 || $('#transaccionesTable').find('input[id*="cuenta_id"]').length > 0 || $('#transaccionesTable').find('input[id*="centro_id"]').length > 0 || element.attr("id") == "totalDebito") {
                    //error.appendTo( $("tfoot td:nth-child(1)") );
                    $("tfoot td:nth-child(1)").html(error);
                }
                else if ($('#cuenta_banco option:selected').length) {
                    $("tfoot td:nth-child(1)").html(error);

                }

                else if ($('#cliente_proveedor').val() == '') {
                    $("tfoot td:nth-child(1)").html(error);
    ...
    calcular_total_debito: function () {
        ...
        $('#retirosForm').find('#totalDebito').prop("value", roundNumber(total_monto, 2));
    },
    calcular_total_debito: function () {
        var total_monto = 0;

        $.each($('#retirosForm').find('#transaccionesTable input[id*="debito"]'), function (i, campo) {
    ...
    eventos: function () {
        ...
        this.botones.guardar.click(function (event) {
            ...
            if (retirosCrear.settings.formulario.valid() === true) {
                $(selfButton).unbind("click");
                $(selfButton).bind("click");
                var guardar = moduloEntradaManual.guardarEntradaManual(retirosCrear.settings.formulario);
...
$(document).ready(function () {
    ...
    setTimeout(function () {
        ...
        if(typeof tipopagoid !== 'undefined'){
        $('#tipo_pago_id').prop('disabled', 'disabled').find('option[value=' + tipopagoid + ']').prop('selected', 'selected');
        $('#tipo_pago_id').trigger("chosen:updated");
      }

      if(typeof numerocheque !== 'undefined'){
        $('.NumeroCheque').removeClass('hide');
        $('.numero_cheque').val(numerocheque).attr('disabled', true);       
      }

      if(typeof nombrebancocheque !== 'undefined'){
        $('.CuentadeBanco').removeClass('hide');
        $('.nombre_banco_cheque').val(nombrebancocheque).attr('disabled', true);       
      }

      if(typeof numerotarjeta !== 'undefined'){
        $('.Numerodetarjeta').removeClass('hide');
        $('.numero_tarjeta').val(numerotarjeta).attr('disabled', true);       
      }

      if(typeof numerorecibo !== 'undefined'){
        $('.Numeroderecibo').removeClass('hide');
        $('.numero_recibo').val(numerorecibo).attr('disabled', true);       
      }

      if(typeof bancoproveedor !== 'undefined'){
        $('.Bancodelproveedor').removeClass('hide');      
        $('.banco_proveedor').prop('disabled', 'disabled').find('option[value=' + bancoproveedor + ']').prop('selected', 'selected');
        $('.banco_proveedor').trigger("chosen:updated");
      }

      if(typeof numerocuentaproveedor !== 'undefined'){
        $('.Numerodecuentadelproveedor').removeClass('hide');
        $('.ach').val(numerocuentaproveedor).attr('disabled', true);       
      }
    ...
    setTimeout(function () {
        $('#cliente_proveedor').trigger('change');
    }, 1000);

//inicio nuevos campos crear #1202
if(vista == 'crear'){
$('#tipo_pago_id').change(function() {
  $('.numero_cheque').val('');
  $('.nombre_banco_cheque').val('');
  $('.numero_tarjeta').val('');
  $('.numero_recibo').val('');
  $('.banco_proveedor').val('').trigger('chosen:updated');
  $('.ach').val('');
  if($('#tipo_pago_id').val() == 19){
    $('.NumeroCheque').show();
    $('.NumeroCheque').removeClass('hide');
    $('.CuentadeBanco').show();
    $('.CuentadeBanco').removeClass('hide');
  ...
  if($('#tipo_pago_id').val() == 20){
    $('.Numerodetarjeta').show();
    $('.Numerodetarjeta').removeClass('hide');
    $('.Numeroderecibo').show();
    $('.Numeroderecibo').removeClass('hide');
  }else{
    $('.Numerodetarjeta').hide();
    $('.Numeroderecibo').hide();
    }
    if($('#tipo_pago_id').val() == 21){
    $('.Bancodelproveedor').show();
    $('.Bancodelproveedor').removeClass('hide');
    $('.Numerodecuentadelproveedor').show();
    $('.Numerodecuentadelproveedor').removeClass('hide');
  }else{
    $('.Bancodelproveedor').hide();
    $('.Numerodecuentadelproveedor').hide();
  }
});
}    
});
