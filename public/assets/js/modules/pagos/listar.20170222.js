       ...
       if(seleccionados.length == 0){
        ...
        aplicarPagosForm.append('<input name="erptkn" value="'+tkn+'" style="display:none;" type="hidden">');
         $.each(seleccionados, function(indice, pago_uuid){
           ...
           aplicarPagosForm.append('<input type="hidden" name="pago['+ indice +'][estado]" value="aplicado" />');
           aplicarPagosForm.append('<input type="hidden" name="pago['+ indice +'][id]" value="'+ id +'" />');
         ...
         if(aprobados == false){
          toastr.warning('Debe seleccionar pagos con estado por aplicar y no sean cheques.');
          return false;
        }
      }
     ...
     $(aplicarPagosModal).on("click", '#cambiarEstadoModalBtn', function(e){
         ...
         $("div.modal-content").find('#cambiarEstadoModalBtn').attr('disabled', true);
          ...
          $.ajax({
              ...
              data: $('#aplicarPagosForm').serialize()+"&multiple=activo",
              type: "POST",
              dataType: "json",
              cache: false,
          }).done(function (json) {
