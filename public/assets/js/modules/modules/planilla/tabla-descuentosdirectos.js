 $(function(){
	...
	$(window).resizeEnd(function() {
		tablaDescuentosDirectos.redimensionar();
	});
});
 //Modulo Tabla de Cargos
var tablaDescuentosDirectos = (function(){

	var url = 'planilla/ajax-listar-descuentos-directos';
	var url_editar =  'planilla/ajax-actualizar-comentario';
	var grid_id = "tablaDescuentosDirectosGrid";
	var grid_obj = $("#tablaDescuentosDirectosGrid");
	...
	var tabla = function(){
 		grid_obj.jqGrid({
			...
			loadComplete: function(data){
				...
				if( data['total'] == 0 ){
					$('#gbox_'+ grid_id).hide();
					$('#'+ grid_id +'NoRecords').empty().append('No se encontraron descuentos directos.').css({"color":"#868686","padding":"30px 0 0"}).show();
				}
				else{
