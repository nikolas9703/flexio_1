...
var formularioCrear = new Vue({
	...
	methods: {
		...
		valorComisionPagada: function(id,com_esperada){
			...
			console.log(com_pagada);
			console.log('holaaaaaa');
			console.log(com_esperada);
			...
			$("input[name='com_pagada[]']").map(function (index,dato) {
				if(dato.value=='')
					...
					valor=dato.value;
				valor_final+=parseFloat(valor).toFixed(2);
			}).get();
			
			$("#com_paga_final_final").html('$'+ valor_final);
		...
		getRemesasProcesadasCancelar: function(){
			...
			else if(estado_remesa=='por_liquidar')
...
$(document).ready(function(){
	...
	if(vista == "editar" && borrador=='no'){
		...
		//formularioCrear.getRemesasPorLiquidar();
