var tablaAnticipos = (function () {
	...
	if (window.location.pathname.match(/ordenes/g)) {
		var a = {'orden_compra': modulo_id};
		window.campo = Object.assign({}, a);
	}
	if(window.location.pathname.match(/proveedores/g)){
		var a = {'proveedor': proveedor_id};
		window.campo = Object.assign({}, a);
	}
	var botones = {
	...
	var tabla = function () {
		...
		var moduloPadre = function () {
			var localstorage = window.localStorage;
			if ((localstorage.getItem("ms-selected") == "compras" || localstorage.getItem("ms-selected") == "contratos") || localstorage.getItem("ml-selected") == "Subcontratos") {
				return 'compras';
			}
			return 'ventas';
		};
		...
		gridObj.jqGrid({
			...
			mtype: "POST",
			datatype: "json",
			colNames: ['', 'No. Anticipo', anticipable, 'Fecha de anticipo', 'Monto total', 'No. Documento', 'Estado', '', ''],
			colModel: [
			...
			postData: {
				...
				modulo_id: moduloId,
				modulo: localstorage.getItem("ms-selected")
			},
			height: "auto",
			autowidth: true,
			...
			loadComplete: function (data, status, xhr) {
				...
				if (data.total === 0) {
					$('#gbox_' + gridId).hide();
					$('#' + gridId + 'NoRecords').empty().append('No se encontraron Anticipos.').css({
						"color": "#868686",
						"padding": "30px 0 0"
					}).show();
				}
	...
	$(botones.limpiar).click(function (e) {
		...
		$('#buscarAnticipoForm').find('select.chosen-select').prop("value", "").trigger('chosen:updated');
	...
	$(botones.buscar).click(function (e) {
		...
		var centro_contable = $("#centro_contable_id").val();
		console.log("Centros:", centro_contable);
			...
			gridObj.setGridParam({
				...
				postData: {
					campo: {
						...
						monto_max: monto_max,
						documento: documento,
						metodo_anticipo: metodo_anticipo,
						estado: estado,
						anticipable_type: anticipable_type,
						cliente: cliente
					},
					modulo: window.localStorage.getItem("ms-selected"),
					erptkn: tkn
				}
			}).trigger('reloadGrid');
