//Modulo
var tablaLiquidaciones = (function(){

	var url = 'configuracion_rrhh/ajax-listar-liquidaciones';
	...
	var tabla = function(){
		...
		$(window).resizeEnd(function() {
			tablaLiquidaciones.redimensionar();
		});
	};
    ...
    //Boton de Duplicar Cargo
		opcionesModal.on("click", botones.editar, function(e){
			...
			var liquidaciones_id = $(this).attr("data-id");
			var rowINFO = grid_obj.getRowData(liquidaciones_id);

		    var liquidacion_id = rowINFO["id"];
		    var nombre = rowINFO["Nombre"];
		    var estado = rowINFO["Estado"];
        estado = $(estado).text();

		    var scope = angular.element('[ng-controller="configLiquidacionesCtrl"]').scope();
        if(estado == 'Activo'){
          estado = 1;
        }else{
          estado = 2;
		   ...
		   $('#liquidacionesForm').find('#estado_id').find('option[value="'+ estado +'"]').prop('selected', true);

		    scope.$apply(function(){
		    	scope.liquidaciones.id = liquidaciones_id;
		    	scope.liquidaciones.nombre = nombre;
		    	scope.liquidaciones.estado_id = estado;
		    });
		    setTimeout(function(){
                    $('#estado_id').trigger('change:updated')
                    }, 300);
		    //Ocultar modal
