...
var tablaSolicitudes = (function(){
	// obtener modulo del local store
	modulo = localStorage.getItem("ml-selected");

	var uuid=$('input[name="campo[uuid]').val();
	var tipo_interes=$('input[name="campo[tipo_id]').val();
	...
	var tabla = function(){
		...
		grid_obj.jqGrid({
		   	...
		   	colModel:[
				...
				{name:'cliente', index:'cliente_id', width: 70},  
				{name:'aseguradora', index:'aseguradora_id', width:40},
				{name:'ramo', index:'ramo_id', width:40},
				{name:'tipo', index:'id_tipo_poliza', width: 35,editable: true, stype:"select", 
				searchoptions: { 
				...
				searchoptions: { 
				value: "0:Todo;Anulada:Anulada;Aprobada:Aprobada;En Trámite:En Trámite;Pendiente:Pendiente;Rechazada:Rechazada", 
								} },
				...
				{name: 'modalanular', index: 'modalanular', align: "center", hidedlg: true, hidden: true},
				{name: 'modalaprobar', index: 'modalaprobar', align: "center", hidedlg: true, hidden: true},
				{name: 'modalarechazar', index: 'modalarechazar', align: "center", hidedlg: true, hidden: true},				
		   	],
			mtype: "POST",
		   	postData: {
		   		erptkn: tkn,
		    ...
		    loadBeforeSend: function () {
                ...
                $(this).closest("div.ui-jqgrid-view").find("#"+grid_id+"_cb, #jqgh_tablaSolicitudesGrid_link").css("text-align", "center");
			}, 
                        ...
                        res.done(function (resp) {
                            ...
                            if (resp == 0) {
                                ...
                                cambio.done(function (response) {
                                    ...
                                    var datosbitacora = {campo: {estado: estado, estado_anterior: estado_anterior, tipo: 'Cambio de Estado', motivo: motivo, solicitud: solicitud}};
                                    var cambiobitacora = moduloSolicitudesBitacora.cambiarEstadoSolicitudesBitacora(datosbitacora);
    ...
    $(botones.exportar).on("click", function (e) {
        ...
        e.returnValue = false;
        e.stopPropagation();
        if ($('#tabla,#tablaSolicitudes,#id_tab_solicitudes').is(':visible') === true) {
//Exportar Seleccionados del jQgrid
            var ids = [];
            ids = grid_obj.jqGrid('getGridParam', 'selarrrow');
            //Verificar si hay seleccionados
            if (ids.length > 0) {
                $('#ids_solicitudes').val(ids);
                $('form#exportarSolicitudes').submit();
                $('body').trigger('click');
