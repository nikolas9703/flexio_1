...
var tablaContactos = (function () {
    var url = 'ajustadores/ajax_listar_contacto';
    var uuid_ajustadores = $('input[name="campo[uuid]').val();
    ...
    var tabla = function () {
        ...
        grid_obj.jqGrid({
            ...
            postData: {
                erptkn: tkn,
                uuid_ajustadores: uuid_ajustadores
            },
            height: "auto",
            autowidth: true,
        ...
        $(botones.exportar).on("click", function (e) {
            ...
            if ($('#tabla').is(':visible') == true) {
                ...
                if (ids.length > 0) {
                    console.log(ids);
                    $('#ids').val(ids);
                    $('#dato_ajustador').val(uuid_ajustadores);
    ...
    $(opcionesModal).on("click", botones.verdetalle, function (e) {
        ...
        var datos = datoscontacto.success(function (data) {
            $.each(data, function (i, filename) {
                ...
                $('input[name="campo[telefono]"').val(filename.telefono);
                $('select[name="campo[estado]"').val(filename.estado);
                $('#impresioncontacto').show();

                $(".breadcrumb").html('<li><a href="../../">Seguros</a></li><li><a href="../listar">Ajustadores</a></li><li><a href="">' + filename.nombre_ajustadores + '</a></li><li class="Active">' + filename.nombre + '</li>');
            });
        });
    });
...
var moduloContactos = (function () {
    return {
        cargardatoscontacto: function (parametros) {
            return $.post(phost() + 'ajustadores/ajax_cargar_contacto', $.extend({
                erptkn: tkn,
                id: parametros
            }, parametros));
        },
        cambiarestadocontacto: function (parametros) {
            return $.post(phost() + 'ajustadores/ajax_cambiar_estado_contacto', $.extend({
                erptkn: tkn,
                id: parametros
            }, parametros));
        },
        cambiarContactoPrincipal: function (parametros) {
            return $.post(phost() + 'ajustadores/ajax_cambiar_contacto_principal', $.extend({
                erptkn: tkn,
                id: parametros
            }, parametros));
        }
