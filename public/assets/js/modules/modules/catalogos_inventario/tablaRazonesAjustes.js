...
var tablaRazonesAjustes = (function(){
    
    // Objeto al cual establecemos valores que vamos a usar mas adelante en este ambito.
    var st = {
        iOptionsModal:"#optionsModal", 
        jqGrid: "#razonesAjustesGrid",
        jqPager: "#razonesAjustesPager",
        optionsModal: "#optionsModal",
        guardarBtn: "#guardarRazonBtn",
        cancelarBtn: "#cancelarRazonBtn",
        noRecords: ".NoRecordsRazonesAjustes",
        segmento2: "catalogos_inventario",
        editar: ".editarRazon",
        desactivar: ".desactivarRazon",
        activar: ".activarRazon",
        //campos del formulario de creacion/edicion
        iNombre: "#razon_nombre",
        iDescripcion: "#razon_descripcion",
        iModo: "#modoRazon",
        cChosen: "#razon_estado"
    };
    ...
    var events = {
        eGuardar: function(e){
            ...
            $.ajax({
                url: phost() + st.segmento2 + "/ajax-guardar-razon",
                type:"POST",
                data:{
                    erptkn:tkn,
                    uuid: uuid,
                ...
                success: function(data){
                    if(data.success === true)
                    {
                        ...
                        toastr.success("¡&Eacute;xito! Se ha guardado correctamente la << Raz&oacute;n/Ajustes >>.");
        ...
        eEditar: function(e){
            ...
            $.ajax({
                url: phost() + st.segmento2 + "/ajax-get-razon",
                type:"POST",
                data:{
                    erptkn:tkn,
                    uuid: uuid
        ...
        eDesactivar: function(e){
            ...
            cambiarEstado(uuid, "7");
        },
        eActivar: function(e){
            var self = $(this);
            var uuid = self.data("uuid");
            
            cambiarEstado(uuid, "6");
        },
    ...
    var muestra_tabla = function(){
        dom.jqGrid.jqGrid({
            url: phost() + st.segmento2 + '/ajax-listar-razones-ajustes',
            datatype: "json",
            colNames:[
                'Nombre',
                'Descripci&oacute;n',
            ...
            loadComplete: function(data){
                ...
                if( data['total'] == 0 ){
                    ...
                    dom.noRecords.empty().append('No se encontraron razones de ajustes.').css({"color":"#868686","padding":"30px 0 0"}).show();
                }
                else{
                    dom.noRecords.hide();
    ...
    var cambiarEstado = function(uuid, estado){
        $.ajax({
            url: phost() + st.segmento2 + "/ajax-cambiar-estado-razon",
            type:"POST",
            data:{
                erptkn:tkn,
                uuid: uuid,
            ...
            success: function(data){
                if(data.success === true)
                {
                    dom.iOptionsModal.modal("hide");
                    dom.jqGrid.trigger("reloadGrid");
                    toastr.success("¡&Eacute;xito! Se ha guardado correctamente el << Raz&oacute;n/Ajustes >>.");
                }
            }
    ...
    var llenarFormulario = function(registro){
        ...
        dom.iModo.data("uuid", registro.uuid_razon);
