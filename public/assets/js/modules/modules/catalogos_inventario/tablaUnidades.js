...
var tablaUnidades = (function(){
    
    // Objeto al cual establecemos valores que vamos a usar mas adelante en este ambito.
    var st = {
        iOptionsModal:"#optionsModal", 
        jqGrid: "#unidadesGrid",
        jqPager: "#unidadesPager",
        optionsModal: "#optionsModal",
        guardarBtn: "#guardarUnidadBtn",
        cancelarBtn: "#cancelarUnidadBtn",
        noRecords: ".NoRecordsUnidades",
        segmento2: "catalogos_inventario",
        editar: ".editarUnidad",
        desactivar: ".desactivarUnidad",
        activar: ".activarUnidad",
        //campos del formulario de creacion/edicion
        iNombre: "#unidad_nombre",
        iDescripcion: "#unidad_descripcion",
        iEstado: "#unidad_estado",
        iModo: "#modoUnidad",
        cChosen: "#unidad_estado"
    };
    ...
    var events = {
        eGuardar: function(e){
            ...
            $.ajax({
                url: phost() + st.segmento2 + "/ajax-guardar-unidad",
                type:"POST",
                data:{
                    erptkn:tkn,
                    uuid: uuid,
                ...
                success: function(data){
                    if(data.success === true)
                    {
                        ...
                        toastr.success("¡&Eacute;xito! Se ha guardado correctamente la << Unidad/Inventario >>.");
    ...
    var muestra_tabla = function(){
        dom.jqGrid.jqGrid({
            url: phost() + st.segmento2 + '/ajax-listar-unidades',
            datatype: "json",
            colNames:[
                'Nombre',
                'Descripci&oacute;n',
    ...
    var cambiarEstado = function(uuid, estado){
        $.ajax({
            url: phost() + st.segmento2 + "/ajax-cambiar-estado-unidad",
            type:"POST",
            data:{
                erptkn:tkn,
                uuid: uuid,
            ...
            success: function(data){
                if(data.success === true)
                {
                    dom.iOptionsModal.modal("hide");
                    dom.jqGrid.trigger("reloadGrid");
                    toastr.success("¡&Eacute;xito! Se ha guardado correctamente el << Unidad/Inventario >>.");
                }
            }
    ...
    var llenarFormulario = function(registro){
        ...
        dom.iModo.data("uuid", registro.uuid_unidad);
