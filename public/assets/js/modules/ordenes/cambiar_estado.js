var change_state_ordenes_compra = function(){
    ...
    var methods = {
        run: function(row){
            ...
            //Init Modal
            if(id){
                data.opcionesModal.find('.modal-title').empty().append('Cambio de estado: '+ $(rowINFO['Numero']).text() +' - '+ $(rowINFO['Proveedor']).text());
            }else{
        ...
        ajaxGetStatesSegment: function(orden_id){
            var aux = data.gridObj.jqGrid('getGridParam','selarrrow');
            var params = $.extend({erptkn:window.tkn},{orden_id: orden_id == -1 ? data.gridObj.jqGrid('getGridParam','selarrrow') :orden_id});
            if(!aux.length && orden_id == -1){
                toastr.error('No se ha seleccionado ning&uacute;n elemento');
                data.opcionesModal.modal('hide');
                return;
            }
            $.ajax({
                url: phost() + "ordenes/ajax_get_states_segment",
                type: "POST",
                data: params,
                dataType: "json",
        ...
        summit:function(btn){
            ...
            data.opcionesModal.modal('hide');
    		$.ajax({
    			url: phost() + "ordenes/ajax_update_state",
    			type: "POST",
