...
var colaboradores = (function(){
	...
	var campos = function(){
	   ...
	   var deduccion_tipo_declarante = $(formulario).find('#deduccion_tipo_declarante_id').val();
       if( deduccion_tipo_declarante != 34){
            deshabiltando_deducciones();
      }else{
        habiltando_deducciones();
      }
/*	 $(".select2").select2({
     width:'100%',
     disabled: true,
   ...
   $(formulario).on('change', '#deduccion_tipo_declarante_id', function(e){
     var   deduccion_tipo_declarante = $(this).val();
     if(deduccion_tipo_declarante == 34){
         habiltando_deducciones();
     }else{
         deshabiltando_deducciones();
			...
			afterAddRow: function(row){
				...
				$(row).find(':input[data-inputmask]').filter(function(){
					return $(this).val() == '';
                                ...
                                $(row).find('.combodate').remove();
                                $(row).find('.fecha').combodate({
                                                                minYear: 1915,
                                                            maxYear: moment().format('YYYY')
                                                        });
      ...
      //popular tipo de salario y salario Mensual
      tipo_salario_mensual(cargo_id);
    ...
    function tipo_salario_mensual(cargo_id) {
      var url = 'colaboradores/ajax-listar-salario';
      //console.log(parametros);
      $.ajax({
    			url: phost() + url,
    			data: $.extend({erptkn: tkn}, {cargo_id:cargo_id}),
    			type: "POST",
    			dataType: "json",
    			cache: false,
        ...
        $.each(data, function(index, element) {
          ...
          $('input[name="campo[tipo_salario]"]').val(element.tipo_rata);

          if(element.tipo_rata == 'Mensual'){
    				//Introducir rata mensual
    				$(formulario).find(campo_salario_mensual).removeAttr('disabled').prop("value", element.rata);

            $(formulario).find("#cuenta_costo_id0").removeAttr('disabled');
            $(formulario).find("#centro_contable_id0").removeAttr('disabled');
            $(formulario).find("#monto_asignado0").removeAttr('disabled');
            $(formulario).find("#prcentaje_distribucion0").removeAttr('disabled');
            $(formulario).find(".eliminarEstudiosBtn").removeAttr('disabled');
            $(formulario).find(".agregarEstudiosBtn").removeAttr('disabled');
            actualizar_chosen();
    			}else{

            $("#distribucionTable tr:nth-child(2)").remove();
            $("#cuenta_costo_id0").prop('disabled', "disabled").find('option:eq(0)').prop('selected', 'selected');
            $("#centro_contable_id0").prop('disabled',"disabled").find('option:eq(0)').prop('selected', 'selected');

            $(formulario).find("#monto_asignado0").attr('disabled', true);
            $(formulario).find("#prcentaje_distribucion0").attr('disabled', true);
            $(formulario).find("#monto_asignado0").val('');
            $(formulario).find("#prcentaje_distribucion0").val('');

            $(formulario).find(".eliminarEstudiosBtn").attr('disabled', true);
            $(formulario).find(".agregarEstudiosBtn").attr('disabled', true);
    				$(formulario).find(campo_rata_hora).removeAttr('disabled').prop("value", element.rata);
            actualizar_chosen();
    			}
		...
		if(typeof colaborador_uuid != 'undefined')
    {
      ...
      //************************* FIN DE VALIDACION **************************************************************
		...
		//Verificar si existe variable salario_mensual
		setTimeout(function(){
			if(typeof selected_departamento_id !== 'undefined' && selected_departamento_id!=""){

				//Seleccionar Departamento
				$(campo_departamento).find('option[value="'+ selected_departamento_id +'"]').prop('selected', 'selected');
  ...
  $("#tipo_deduccion_id0").find('option:eq(13)').prop('selected', 'selected');
//	$("#tipo_deduccion_id0").val(13);
    $('#tipo_deduccion_id0').chosen({width: '100%'}).trigger('chosen:updated');
	...
	var popular_departamento = function(centro_id, seleccionado)
	{
		...
		//Buscar listado de Areas de Negocios
		/*/Asociadas al Centro contable seleccionado.
		ajax('colaboradores/ajax-lista-departamentos-asociado-centro', {centro_id: centro_id}).done(function(json){console.log("Response lista_depoartamenteo:",json);
  ...
  var habiltando_deducciones = function() {
        $(formulario).validate({
        focusInvalid: true,
        ignore: '',
        wrapper: '',
      });
          $(formulario).find('#tipo_deduccion_id0, #nombre0, #cedula0, #edad0, #relacion_id0, #deduccion0').rules(
              "add",{
                required: true
          });
          $("#deduccion0").val("800.00");
          $("#totalDeduccion").val("800.00");
            $(formulario).find('#tipo_deduccion_id0').val(13);
          $(formulario).find('#tipo_deduccion_id0,#deduccion0,#totalDeduccion').attr('disabled', true);
         $(formulario).find('#nombre0, #cedula0, #edad0, #relacion_id0').attr('disabled', false);
         actualizar_chosen();

  };

  var deshabiltando_deducciones = function() {
        $(formulario).validate({
        focusInvalid: true,
        ignore: '',
        wrapper: '',
    ...
    $(formulario).find('#tipo_deduccion_id0').val('');
    $("#deduccionesTable").find('#tipo_deduccion_id0, #nombre0, #cedula0, #edad0, #relacion_id0, #deduccion0,#totalDeduccion').val('');
       $("#deduccionesTable").find("input,button,textarea,select").attr("disabled", "disabled");
         //$(formulario).find('#tipo_deduccion_id0, #nombre0, #cedula0, #edad0, #relacion_id0, #deduccion0,#totalDeduccion').attr('disabled', true);
